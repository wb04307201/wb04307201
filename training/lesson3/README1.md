...
1. 前往 [Spring Initializr](https://start.spring.io/) 创建项目，选择以下选项：
   - **Project**: Maven
   - **Language**: Java
   - **Spring Boot**: 3.x（推荐最新稳定版）
   - **Java**: 17+

2. 添加核心依赖 `spring-boot-starter-web`（或通过 IDE 直接勾选 Spring Web）：
```xml
    <dependency>
...
4. 运行 `mvn spring-boot:run` 或在 IDE 中运行主类，确认应用能正常启动。
