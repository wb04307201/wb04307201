...
2. **构造器注入还是字段注入？** Spring 官方推荐**构造器注入**（强制依赖、利于测试、支持 final）。👉 详见 [为什么不推荐 @Autowired 字段注入](../../../note/13.split-hairs/04.spring-backend/not-use-@autowired/README.md)。
...
- [13 辨析/为什么不推荐 @Autowired 字段注入](../../../note/13.split-hairs/04.spring-backend/not-use-@autowired/README.md)
