...
---
## 引言：HTTP 协议演进 — 从 1.0 到 3 的 30 年

HTTP 协议不是一次性设计，是**浏览器、服务器、CDN、TLS 库、网络协议栈反复博弈的产物**。HTTP/0.9 只支持 GET，HTTP/1.0 引入方法+头+状态码但每请求一连接，HTTP/1.1 持久连接 + 流水线，HTTP/2 二进制分帧 + 多路复用，HTTP/3 把 TCP 换成 QUIC 解决队头阻塞。本篇按时间线，对比 5 个关键版本的设计取舍。
