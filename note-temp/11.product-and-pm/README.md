...
本章节是 11 主模块中「产品 + PM」主题的入口，按 **9 大实战场景** 收纳项目决策类内容，覆盖报价拆解、外包避坑、技术选型、AI 研发效能、人力排期、组织拓扑、面试评估、风险管控、敏捷度量。
...
| 维度 | `note/04.system-design/` | `note/13.split-hairs/` | **本模块 `11.product-and-pm/`** |
|------|---------------------------|------------------------|----------------------------------------|
...
| 维护脚本 | 2 | [insert-frontmatter.py](./scripts/insert-frontmatter.py) / [validate.py](./scripts/validate.py) |
| SPEC.md | 1 | pre-existing 占位（未触动） |
| **README 总数** | **12** | 1 顶层 + 9 子 + 1 cheatsheet + 1 scripts |
| **目录总文件数** | **14** | 11 .md + 2 .py + 1 SPEC.md |
