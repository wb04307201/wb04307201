...
**2026 趋势**：Serverless Workflow 1.0 GA + CNCF 成熟 + 与 AI 编排融合（详见 [11.ai 编排平台](../../../../../note/11.ai/03-engineering/ai-platforms/README.md)）
...
> ⚠️ **数据声明**：本节基于公开技术分享整理，12306 真实峰值/QPS/容灾切换时间等精确数字缺乏可点击官方来源。下表数字仅作"假设性架构示例"，请勿作为生产决策依据。
...
- 事件丢失率显著降低
- 新业务接入时间从周级降到天级（仅需订阅 Topic）
- 跨机房容灾切换时间从分钟级降到秒级

> 💡 12306 是 **CloudEvents + EventMesh + Serverless Workflow** 三大事件标准协同落地的标杆案例（架构示意，非官方实测报告）。
...
5. **事件驱动工作流的最大风险？** **事件丢失 / 重复消费 / 顺序错乱**——必须有**幂等设计**（详见 [04 系统设计/06 幂等](../../../../../note/04.system-design/06-idempotency/README.md)）+ **死信队列** + **事件版本管理**。
6. **12306 早期为什么不用 EventMesh？** EventMesh 于 2023 年从 Apache 孵化器毕业成为顶级项目（TLP），12306 2012 上线时 EventMesh 尚未存在，当时是自研事件总线。**历史包袱 + 业务稳定** 后才逐步引入新基础设施——这是国内大型系统的典型演进路径：自研 → 标准化 → 生态化。
...
- [04 系统设计/02 分布式](../../../../../note/04.system-design/02-distributed/README.md) — CAP/共识理论基础
- [04 系统设计/06 幂等](../../../../../note/04.system-design/06-idempotency/README.md) — 事件驱动必配的幂等设计
