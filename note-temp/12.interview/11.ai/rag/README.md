...
> 一句话定位：RAG（Retrieval-Augmented Generation）让 LLM 基于私有/实时数据回答 —— 是 2026 年企业 LLM 应用的事实标准。完整概念见 [主模块 RAG vs Fine-tuning](../../../../note/11.ai/08-llmops/01-rag-vs-finetuning/README.md)。
...
- 主模块：[RAG vs Fine-tuning](../../../../note/11.ai/08-llmops/01-rag-vs-finetuning/README.md)
...
- 深度原理：[Agentic Search vs RAG](../../../../note/11.ai/02-technology-stack/agentic-search-vs-rag/README.md)
...
## 十、RAG 演进四阶段（Naive → Advanced → Modular → Agentic）

> 🔗 深度原理：[`RAG 范式演进四阶段`](../../../../note/11.ai/02-technology-stack/rag-paradigm-evolution/README.md) — 每阶段核心能力/关键技术/局限 + 对比表 + 选型决策树 + 与 5 阶段 Pipeline 正交关系澄清

**面试高频问法**："RAG 有哪几代？怎么演进的？" —— 一句话记忆四阶段核心能力：

| 阶段 | 核心能力 | 一句话 | 关键技术 |
|------|---------|-------|---------|
| **Naive RAG** | 基础连通与快速验证 | **"能跑通"** | 索引→检索→生成 单向线性 |
| **Advanced RAG** | 检索精度与生成质量优化 | **"更精准"** | 查询重写 / 混合检索 / Rerank / 上下文压缩 |
| **Modular RAG** | 组件解耦与动态编排 | **"更灵活"** | Router/Memory/Search 模块 + 条件/迭代/递归编排 |
| **Agentic RAG** | 自主决策与任务规划 | **"会思考"** | Agent 意图拆解 + 多步工具调用 + 自我反思 |

### 陷阱 6：以为四阶段是替代关系
- **真相**：**继承与发展**，不是替代。Naive 在原型验证场景仍最优；简单问答用 Agentic = 杀鸡用牛刀（延迟/成本翻倍）。工程共识：**Advanced + Modular 组合**是高性价比基线，Agentic 按需局部引入。

### 陷阱 7：把"演进四阶段"和"5 阶段 Pipeline"混为一谈
- **真相**：两者**正交**。四阶段是**代际**（能力成熟度），5 阶段 Pipeline（Query Rewrite→Hybrid→Rerank→Compression→Generation）是**单次执行机制** —— 后者正是 Advanced RAG 的典型工程实现。

### 30 秒话术
> RAG 是一条演进主线，核心能力依次递进：**Naive"能跑通"**（索引-检索-生成基线）→ **Advanced"更精准"**（查询重写+混合检索+Rerank，降幻觉）→ **Modular"更灵活"**（组件解耦、动态编排、适配多源异构）→ **Agentic"会思考"**（Agent 自主拆解任务、多步调用、自我反思）。四者是继承而非替代，落地通常用 Advanced+Modular 做基线，复杂多跳场景再引入 Agentic。
...
- [36-rag-retrieval-augmented-generation](../../../../note/12.story/36-rag-retrieval-augmented-generation.md)
