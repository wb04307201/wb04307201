...
> 一句话定位：**单 Agent Memory = 私有上下文；多 Agent 共享记忆 = 跨进程的状态/事实/语义同步，本质是分布式系统问题**。完整深度 + 6 大模式 + 一致性协议见 [主模块 · agent-memory 共享专章](../../../../note/11.ai/04-architecture/agent-memory/shared-memory.md)。
...
- [11.ai · agent-memory 共享专章](../../../../note/11.ai/04-architecture/agent-memory/shared-memory.md) — 5 大维度详解 + 3 层架构 + 6 模式实战 + 一致性协议 + 选型决策
