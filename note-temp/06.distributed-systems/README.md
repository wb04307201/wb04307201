...
> 分布式系统 + 微服务 + 云原生完整图谱：从基础理论（DDD / TOGAF）到工程实践（分布式 / 高可用 / 高性能 / 安全 / 幂等 / 部署 / 可观测性 / 新兴技术）的完整路径。
...
- 上游：[`01.java-and-jvm`](../01.java-and-jvm/README.md) — 并发 / I/O / 网络编程为分布式系统提供底层支撑
- 上游：[`02.cs-foundations`](../02.cs-foundations/README.md) — 网络协议、Linux 基础
- 上游：[`03.data-stack`](../03.data-stack/README.md) — 事务、索引、缓存、连接池（数据层核心）
- 关联：[`04.spring-backend`](../04.spring-backend/README.md) — Spring 全家桶（分布式系统的 Java 实现）
- 关联：[`07.devops-and-tools`](../07.devops-and-tools/README.md) — Docker / Nginx / 监控告警（基础设施）
- 关联：[`05.frontend`](../05.frontend/README.md) — 前端架构（BFF / 微前端 / 渲染模式）
...
- **README 覆盖**：113 个（顶层 9 + depth-3 leaf 54 + depth-4 leaf 37 + depth-5 leaf 12）
- **frontmatter 覆盖**：113/113 = 100%
- **本版本 → 上一版本**：
  - 从 `note/04.system-design` 迁移到 `note-temp/06.distributed-systems`
  - 模块定位升级：系统设计 → 分布式系统 + 微服务 + 云原生
  - 顶层 README 路径更新：`01.java` → `01.java-and-jvm`，`02.computer-basics` → `02.cs-foundations` 等
