...
> **系列定位**：本篇是「阿明餐厅」系列的**续集十五**。在[续集十二 · 36a 成本结构](./34a-ai-token-cost-structure.md)2.2-2.3 节，我们讲了 Embedding 成本与向量库成本。在[续集十四 · 38 · RAG 专题](./36-rag-retrieval-augmented-generation.md)第一章，我们讲了向量检索是 RAG 的核心环节。本篇是**向量数据库与 Embedding 实战专题** —— 从选型到部署，从性能到成本，从单库到分布式，完整讲透向量技术栈。
...
### 5.1 成本结构（详见 [36a 第二章 2.3](./34a-ai-token-cost-structure.md#23-组件-3向量数据库成本占-5-10)）
...
详见 [38 第二章 2.1](./36-rag-retrieval-augmented-generation.md#21-模式-1hybrid-search混合检索)
```
...
详见 [37 · AI 可观测性](./35-ai-observability.md)
```
...
**本节定位**：本章是 12.story/37 味道仓库的**深度专题章节**，聚焦 10 亿级向量检索的算法选型。
...
完整内容见 [11.ai/02-technology-stack/vector-search-algorithms](../../note/11.ai/02-technology-stack/vector-search-algorithms/README.md) 与 [咬文嚼字·11.ai/vector-search-algorithms（面试版）](../../note-temp/12.interview/11.ai/vector-search-algorithms/README.md)。
...
**关键洞察**：HNSW 在 10 亿级**内存放不下**（4TB 单机放不下），必须换 DiskANN。详细原理、参数调优、业界库对比、3 个生产案例（电商/RAG/图像搜索）见 [11.ai/02-technology-stack/vector-search-algorithms](../../note/11.ai/02-technology-stack/vector-search-algorithms/README.md)。

---

## 第九章：10亿 vs 千亿级向量检索——5 个架构转变

**本节定位**：本章是 12.story/37 味道仓库的**千亿级专题**——承接第八章（10亿级选型），讲清楚 10B → 100B → 1000B 的**质变**。
...
完整内容见 [11.ai/02-technology-stack/vector-search-at-scale](../../note/11.ai/02-technology-stack/vector-search-at-scale/README.md) + [咬文嚼字·11.ai/vector-search-at-scale（面试版）](../../note-temp/12.interview/11.ai/vector-search-at-scale/README.md) + [第八章反向链（10亿级）](./README.md#第八章10亿级毫秒级检索hnsw-vs-ivf-vs-diskann-选型逻辑)。
...
## 第十章：万亿级向量检索——多集群联邦 + 专用硬件 + 边缘计算

**本节定位**：本章是 12.story/37 味道仓库的**万亿级专题**——承接第九章（千亿级 vs 10亿级），讲清楚 1T+ 级别的架构。
...
完整内容见 [11.ai/02-technology-stack/vector-search-trillion](../../note/11.ai/02-technology-stack/vector-search-trillion/README.md) + [咬文嚼字·11.ai/vector-search-trillion（面试版）](../../note-temp/12.interview/11.ai/vector-search-trillion/README.md) + [第九章反向链（千亿级 vs 10 亿级）](./README.md#第九章10亿-vs-千亿级向量检索5-个架构转变)。

### 10.2 5 个新挑战速览

1. **多集群 + 跨数据中心联邦**（10-100 个集群，跨集群 50-100ms）
2. **专用硬件**（TPU / 定制 ASIC，不是堆 GPU）
3. **边缘计算 + 端侧缓存**（命中 50%+ 用户请求，< 10ms）
4. **极限压缩**（接受 Recall 90-95%，用 1-bit / PQ4）
...
万亿级瓶颈**不是延迟**，而是**召回 + 成本 + 一致性 + 工程复杂度**四元权衡：

- 真正 5ms 只对 hot tier（5% 数据）有意义
- 万亿级用户感知延迟 27ms（边缘 cache 命中）
...
- [RAG 检索增强生成 38](./36-rag-retrieval-augmented-generation.md) —— 续集十四，向量检索是 RAG 的核心环节
- [AI 成本结构 36a](./34a-ai-token-cost-structure.md) / [36b 成本优化](./34b-ai-token-cost-optimization.md) —— 续集十二，向量库与 Embedding 成本详解
- [AI 可观测性 37](./35-ai-observability.md) —— 续集十三，向量检索的 trace 与监控
- [AI 评测工程 34a/34b](./32a-ai-evaluation-fundamentals.md) / [34b](./32b-ai-evaluation-pipeline.md) —— 续集十，向量检索的 Recall / MRR 评测
