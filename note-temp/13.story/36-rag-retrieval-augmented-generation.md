...
> **系列定位**：本篇是「阿明餐厅」系列的**续集十四**。在[正传 4 · 《厨房质检员》](./08-qa-testing-strategy.md)第四章，我们提到了 RAG 的"忠实性"维度。在[续集六 · 《AI 的"黑暗料理"》](./28-ai-hallucination-safety.md)，我们提到过 RAG 是抑制幻觉的主要手段。在[34b · AI 评测流水线](./32b-ai-evaluation-pipeline.md)第六章，我们讲了 RAG 评测的 4 大指标。本篇是 **RAG 专题** —— 从架构到优化、从基础到高级模式、从离线评测到生产化，完整讲透 RAG。
...
> **阿明的厨房笔记（开篇场景）**：阿明翻遍了所有文档，**从未记录过红烧肉的热量数据**。但 AI 自信地回答："红烧肉 100g 含 480 千卡"，编造得头头是道。阿明意识到 —— AI 厨师不是不能做菜，而是没看菜谱就敢做。本章阿明要给"AI 厨师"配一本"实时翻开的菜谱" —— 这就是 RAG（检索增强生成）的本质。
...
> **阿明的厨房笔记（第二章场景）**：基础版 RAG 跑起来了，但阿明发现三个问题：（1）顾客问"川菜"只搜菜名，搜不到"辣的食物"（语义缺失）；（2）菜谱太多翻不过来，每次都翻 100 页（召回过多）；（3）AI 看到的是片面的菜单信息（缺乏关系）。本章阿明请来了"7 位高级厨师" —— 7 种高级 RAG 模式，每位厨师都有自己的"翻菜谱绝活"。
...
> **阿明的厨房笔记（第三章场景）**：菜谱翻到了，但 AI 厨师还是经常看错 —— 顾客问"红烧肉的盐量"，AI 看到了红烧肉的"做法"段落，没看到"营养成分"段落。本章阿明发现，菜谱不是简单"翻到就行"，还要"翻得准"：字号要合适（chunk size）、书签要精准（chunk 边界）、重点要标注（关键信息加权）、过滤要聪明（元数据过滤）。本章讲 RAG 调优的 5 个"翻菜谱的细节"。
...
> **阿明的厨房笔记（第四章场景）**：调优完了，阿明以为万事大吉，结果 AI 厨师又翻车了 —— 顾客投诉"红烧肉吃起来太咸"。老陈一查：原来 RAG 翻到了"红烧肉的常规做法"，但漏掉了"减盐版红烧肉"（明明在另一页）；AI 看到了盐量数据，但以为顾客要"标准做法"（没结合用户偏好）。本章阿明总结"AI 厨师翻车"的 4 大典型场景 —— 都是真实顾客投诉复盘。
...
> **阿明的厨房笔记（第五章场景）**：菜谱翻得准不准、菜做得对不对，要怎么测？阿明以前靠"试吃员拍脑袋打分"，现在阿明要给 AI 厨师配"数字化打分表"：菜谱是否翻到（召回率）、翻到的内容是否有用（精度）、做出的菜是否对（忠实性）、顾客是否满意（体验）。本章讲 RAG 评测的"四维度试吃打分"。

## 第五章：RAG 评测体系 —— 四维度试吃打分，查得准不准做得像不像

### 5.1 4 大核心指标（详见 [34b 第六章](./32b-ai-evaluation-pipeline.md#第六章rag-系统的专项评测)）
...
> **阿明的厨房笔记（第六章场景）**：试吃合格了，阿明准备把"AI 厨师"推到 80 家分店。结果一看账单 ——"每月翻菜谱 + 做菜 = 8 万，比雇 5 个真厨师还贵！"本章阿明要把"试菜间"升级成"大灶厨房" —— 处理真实顾客的并发请求、控制成本、保证 7×24 可用。生产化的关键不是"能跑"，而是"跑得起、跑得稳、跑得便宜"。
...
详见 [36a 第二章 2.1 LLM 推理成本](./34a-ai-token-cost-structure.md#21-组件-1llm-推理成本占-60-80)
```
...
RAG 系统需要专门的 RAG Observability（详见 [37 · AI Observability](./35-ai-observability.md)）：
...
> **阿明的厨房笔记（第七章场景）**：AI 厨师从"瞎炒"到"翻菜谱做菜"，已经成了 2026 年的行业标准。但老陈说："老板，下一个版本，AI 厨师会看图做菜 —— 看一眼顾客发来的红烧肉照片，直接告诉你做法。"RAG 正在进化 —— 从"文字检索"到"多模态检索"（图、音、视频）。本章展望 RAG 的下一个 3 年。
...
- [AI 的"黑暗料理"](./28-ai-hallucination-safety.md) —— 续集六，RAG 是抑制幻觉的主要手段
- [AI 评测工程 34a/34b](./32a-ai-evaluation-fundamentals.md) / [34b](./32b-ai-evaluation-pipeline.md) —— 续集十，RAG 评测的 4 大指标详解
- [AI 可观测性 37](./35-ai-observability.md) —— 续集十三，RAG 的可观测性
- [AI 成本结构 36a](./34a-ai-token-cost-structure.md) / [36b](./34b-ai-token-cost-optimization.md) —— 续集十二，RAG 的成本与优化
- [向量数据库与 Embedding 39](./37-vector-database-and-embedding.md) —— 39 续集十五，RAG 的向量库与 Embedding 详解
...
- 范式代际：[RAG 范式演进四阶段](../09.ai-applications/rag/02-paradigm-evolution.md) — Naive→Advanced→Modular→Agentic，本文的 5 环节 + 7 模式即 Advanced/Modular 阶段的落地
