...
> **系列定位**：本篇是「阿明餐厅」系列的**续集十七**。在[续集十六 · 40 · AI 合规](./38-ai-compliance-and-regulation.md)我们讲了数据出境的合规要求。在[续集十二 · 36a 成本结构](./34a-ai-token-cost-structure.md)我们讲了 LLM API 的成本。本篇是**AI 模型私有化部署专题** —— 当你不能或不想用 OpenAI API 时，怎么在自有环境部署 LLM。从单 GPU 到分布式，从量化到微调，从性能到成本。
...
- [AI 成本结构 36a](./34a-ai-token-cost-structure.md) / [36b 成本优化](./34b-ai-token-cost-optimization.md) —— 续集十二，私有化 vs API 成本对比
- [AI 合规与监管 40](./38-ai-compliance-and-regulation.md) —— 续集十六，数据出境的合规要求
- [可观测性 37](./35-ai-observability.md) —— 续集十三，私有化部署的监控
- [RAG 38](./36-rag-retrieval-augmented-generation.md) —— 续集十四，私有化 RAG 系统
- [向量数据库 39](./37-vector-database-and-embedding.md) —— 续集十五，私有化向量库

---


- [02-rag-in-agent](../../note/11.ai/04-architecture/agent-context/02-rag-in-agent.md)
...
- 续集十八：[`44-ai-engineer-responsibility`](./42-ai-engineer-responsibility.md) —— **私有化场景下的责任分工**：私有化后谁负责运维、谁负责安全、谁负责更新
- 续集十九：[`45-ai-productivity-paradox`](./43-ai-productivity-paradox.md) —— **私有化的 ROI 真相**：DORA 放大器理论告诉你私有化是否值得
- 续集十二：[`36b-ai-token-cost-optimization`](./34b-ai-token-cost-optimization.md) —— **私有化的成本优化**：5 层路由 + 3 级缓存 + 4 策略压缩

> 🆕 **阿明餐厅工业部署选型实战**：当连锁开到 30 家（从私部署 Ollama 升级到 vLLM），怎么选推理引擎？→ 详见 [vLLM vs Ollama 工业级选型深度专题](../../note/11.ai/03-engineering/ai-platforms/vllm-vs-ollama/README.md)（PagedAttention 原理 + 4 引擎对比 + 场景化决策树）。
