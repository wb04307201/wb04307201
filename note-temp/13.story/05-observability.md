...
## 阿明的"AI 思维工程 5 问"：装监控之后，CTO 还要回答的 5 个灵魂拷问

接上一篇的可观测性三支柱上线后，阿明的 AI 客服业务又涨了 3 倍——但新问题来了。

业务总监在评审会上甩出 5 个问题，每一个都让 CTO 阿明头大：
...
这 5 个问题不是写好 Prompt 能解决的——是 **LLM Production Engineering** 的全部。

**第 1 问**，阿明栽过跟头：产品同事把"金额校验"都丢给 LLM，结果 5ms if-else 变 500ms + $0.01/次，还错 5%——用户付了两次钱。教训：**能用规则就别用 LLM（成本 ×5、错误率从 0% 涨到 5%）**，4 信号决策（结构化？规则可枚举？错误容许？ROI ≥ 10x？）。

**第 2 问**，上线的第一周就踩雷了：有个 Agent 循环 bug，token 无上限调用，第一天晚上烧了 $4200。教训：**3 道 quota（单请求 / 单用户 / 单租户）+ 5 层路由**（缓存 → cheap 模型 → big 模型 → SaaS 兜底 → 人工介入）。95% 阈值熔断，强行终止。

**第 3 问**，客服问"北京人口多少？"3 次返回 2000/3000/5000 万——重试没用，因为是模型概率性。教训：**Self-Consistency 投票（5 次采样）+ Judge 模型选最佳 + 重试预算分级**（网络错重试 3 次，幻觉不能重试改投票）。

**第 4 问**，供应商 OpenAI 短暂故障 30 分钟，APP 完全卡死，每个请求卡 60 秒。教训：**双 timeout（5s 软限 partial + 30s 硬限 fallback）+ Circuit Breaker 熔断 30s + Fallback 模型链**（GPT-4o → Claude → Llama-3 本地 → static FAQ）。

**第 5 问**，上游 GPT-4o 静默升级权重，用户狂喷"AI 变笨了"——但阿明 7 天才察觉。教训：**Trace 链路追踪（5 分钟定位 vs 3 天排查）+ 黄金集回归（每月 / 每次升级）+ 漂移检测**（Embedding / Prompt / 召回率）。

3 个月后，阿明的 AI 客服业务：

- **P99 延迟**：5s → 1.2s
- **成本**：日烧 $4200 → $320（降 92%，多亏 5 层路由 + 熔断）
- **准确率**：72% → 92%（黄金集回归 + Self-Consistency 投票）
- **故障恢复**：小时级 → 分钟级（Trace + 漂移告警）

**关键教训**：阿明把"5 问"写进了团队 wiki 新人入职清单——**每个 AI 工程师入职第一周必修课，是回答这 5 个问题能落地。**

> 🆕 **5 问完整技术解析**：Prompt vs if-else 4 信号决策 + 5 层路由 + Self-Consistency + 双 timeout + Trace 监控 → 详见 [大模型思维工程专题](../../note/11.ai/08-llmops/production-stability/README.md)（含 7 文件 1512 行）。面试精选 5 题（拆分独立版）见 [思维范式](../../note-temp/12.interview/11.ai/llm-thinking-paradigm/) · [成本控制](../../note-temp/12.interview/11.ai/llm-cost-control/) · [一致性](../../note-temp/12.interview/11.ai/llm-consistency/) · [超时熔断](../../note-temp/12.interview/11.ai/llm-timeout-circuit-breaker/) · [监控](../../note-temp/12.interview/11.ai/llm-monitoring/)。
