"""Verify Task 10 link fix matches Task 9 pattern.

Checks:
  1. ZERO cross-module links (../X.Y./ where X.Y. is module prefix) WITHOUT note/ prefix
  2. ZERO sibling 13.split-hairs/ links remain (should be converted to direct sibling)
  3. Pre-existing broken count (target doesn't exist) compared to source baseline

Note: Uses os.path.exists (accepts both files and directories) since Obsidian and other
renderers auto-resolve directory refs to <dir>/README.md.
"""
import os
import re
import sys
...
def collect_links_from_ref(ref: str, path_prefix: str) -> tuple[int, list]:
    ...
    for rel_f in files:
        ...
        for m in LINK_RE.finditer(content):
            rel = m.group(1)
            total += 1
            # Resolve to absolute path (ref content as baseline)
            target = os.path.normpath(os.path.join(os.path.dirname(rel_f), rel)).replace(os.sep, "/")
            # Convert to working-tree path: file is at PROJECT_ROOT/target
...
# === Check 2: Sibling 13.split-hairs/ links should be removed ===
print()
print("=== Check 2: Sibling 13.split-hairs/ links should be removed ===")
sib_links = []
for md in glob.glob(os.path.join(TARGET_DIR, "**/*.md"), recursive=True):
    with open(md, "r", encoding="utf-8") as fp:
        for ln, line in enumerate(fp, 1):
...
print(f"Sibling 13.split-hairs/ links remaining: {len(sib_links)}")
if sib_links:
...
# === Check 3: Pre-existing broken count (against git HEAD~1 baseline) ===
print()
print("=== Check 3: Broken link count vs source baseline (git HEAD~1) ===")
