...
### Step 1: 现状扫描（必做，2026-07-14 精简为 8 步）
...
### Step 5.5: Commit 拆分模式（2026-07-14 新增）
...
**示例（2026-07-14 第 1 轮审计拆分 18 commit）**：
...
| `数字对吗？` | 只跑 Step 1.3（数字一致性） + Step 2 过滤 + Step 3 分级 |
| `链接有问题吗？` | 只跑 Step 1.6（broken links） + Step 4 机械分类 |
| `重复内容多吗？` | 只跑 Step 1.7（浅 README / 重复检测） + Step 3 P2 分类 |
| `哪里缺内容？` | 只跑 Step 1.7（浅 README） + Step 3 P1 分类 |
| `全部执行` / `一起做` | Step 5 → Step 6 直接执行 Mode B（跳过选项） |
| `拆分做 commit` | Step 5.5 commit 拆分模式 |
...
### ❌ Mistake 8.5: Windows 行尾污染（2026-07-14 新增）
...
### ❌ Mistake 8.6: 误删用户配置（2026-07-14 新增）
...
- [ ] **Step 5.5（用户说"拆分 commit"时）**：commit 拆分模式
- [ ] **Step 5.6 风险检查**：6 项必过（不删 .obsidian 等）
