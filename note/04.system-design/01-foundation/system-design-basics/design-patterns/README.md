...
GoF设计模式/23种设计模式 的**GoF（Gang of Four）设计模式，即《设计模式：可复用面向对象软件的基础》一书中提出的23种经典设计模式，可分为创建型、结构型和行为型三大类。**
...
graph TD
    GoF["GoF 23 种设计模式"]
    GoF --> Create["创建型（5 种）<br/>对象创建过程"]
    GoF --> Struct["结构型（7 种）<br/>类与对象组合"]
    GoF --> Behave["行为型（11 种）<br/>对象间交互"]
