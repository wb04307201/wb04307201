...
数据库分库分表 本应该很简单，分库分表是应对单库单表数据量过大、并发压力过高的终极方案。本文系统讲解垂直/水平分库、垂直/水平分表,以及分片算法与最佳实践
...
5. 数据迁移与扩容
   随着数据量增长，分片需要重新分布（例如从 4 库扩容到 8 库），需要专门的迁移工具（如 ShardingSphere-Scaling）保证数据一致性和迁移期间的可用性。
