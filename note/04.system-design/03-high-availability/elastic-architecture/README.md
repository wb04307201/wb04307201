...
弹性架构的核心矛盾不是"要不要自动扩缩"——而是**扩缩速度、容量预测偏差、成本三者的三角平衡**。HPA 响应滞后会丢流量；预测式扩容在突发场景下严重浪费；Serverless 看似免运维但冷启动代价不可忽视。本篇围绕"被动响应 vs 主动预测"、"纵向扩 vs 横向扩"、"云原生 vs 自建"三条主线，把 5 大核心 trade-off 讲清楚。
