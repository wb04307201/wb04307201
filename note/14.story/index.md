...
| **按成熟度主线** | 前传（含 L1-L7 评估）→ 正传 1 → 正传 6 → 正传 11 → 番外三 → 正传 14 → 正传 15 | 从 L1 起步，一步步走到 L6-L7：架构演进 → 流量治理 → API 服务化 → 异步解耦 → 多租户 → 多端统一 → 零停机迁移 |
...
| 成熟度评估 L1-L7 | 前传第八章 | 正传 6（服务化思维 → L4）、正传 11（异步解耦 → L5）、正传 14（多端统一 → L6）、番外三（多租户 → L5-L6）、正传 15（零停机 → L6）|
