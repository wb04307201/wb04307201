...
> 用开餐厅的故事，讲明白二十六件技术大事。
> 从架构演进到 AI 智能体，从流量治理到测试策略，从 CI/CD 到团队管理，从数据架构到前端工程化，从故障应急到性能优化，从消息队列到分布式难题，从多租户 SaaS 到国际化部署 —— 一篇一个核心主题，篇篇独立又互相串联。
...
graph LR
    ...
    subgraph 正传
        ...
        K[数据架构<br/>数据仓库/ETL/数据治理]
        L[前端工程化<br/>Core Web Vitals/组件化/状态管理]
        M[故障应急<br/>Runbook/复盘/混沌工程]
        N[性能优化<br/>USE 方法/缓存/并发]
        Q[消息队列<br/>异步解耦/削峰填谷]
        R[分布式难题<br/>CAP/锁/幂等]
        S[实时系统<br/>WebSocket/CDC/事件溯源]
        T[多端架构<br/>BFF/跨平台/离线优先]
        U[数据库迁移<br/>在线 DDL/双写/分库分表]
    end
    subgraph 续集
        H[AI Agent<br/>感知→记忆→规划→工具]
        O[AI 学习<br/>认知卸载/刻意练习/人机协作]
    end
    subgraph 终章
        I[团队管理<br/>5人→500人]
    end
    subgraph 番外
        J[重构决策<br/>技术债→渐进式翻新]
        P[云成本<br/>FinOps/右Size 化/成本可视化]
        V[多租户 SaaS<br/>数据隔离/计费模型]
        W[搜索推荐<br/>倒排索引/协同过滤]
        X[知识工程<br/>SECI/ADR/Docs-as-Code]
        Y[低代码平台<br/>标准化 vs 灵活性]
        Z[国际化<br/>i18n/合规/多币种]
    end
    A --> B
    A --> C
    A --> D
...
### 正传 14：[一个厨房四个门面](./21-multiplatform-architecture.md)
...
- [术语表](./glossary.md) —— 130+ 核心技术术语速查，按 25 大主题分类
- [一页纸速查](./cheatsheet.md) —— 26 篇文章的核心概念、关键表格、金句心法汇总
...
| 安全工程师 | 正传 3 → 正传 5 → 续集一第七章 → 正传 9 → 番外七 | 系统安全 → 安全左移 → AI 安全 → 故障应急 → 数据合规 |
...
| AI 工程师 | 续集一 → 续集二 → 前传 → 正传 6 → 正传 3 → 正传 13 | Agent 全景 → 人机协作 → 架构基础 → API → 安全护栏 → 实时事件驱动 |
| 架构师 | 前传 → 续集一 → 正传 1/2/3/6/11/12 → 番外三 → 终章 | 从架构到 AI 到工程到分布式到 SaaS 到管理，全局视角 |
| 产品经理 | 番外一 → 番外二 → 番外六 → 前传 → 正传 9 → 番外四 | 重构决策 → 云成本 → 低代码 → 架构全貌 → 故障复盘 → 搜索推荐 |
| CTO / VP | 终章 → 番外二 → 番外三 → 前传 → 续集一 → 正传 5 | 组织管理 → 云成本 → SaaS 架构 → 架构 → AI → 交付效率 |
