...
> 用开餐厅的故事，讲明白四十件技术大事。
> 从架构演进到 AI 智能体，从流量治理到测试策略，从 CI/CD 到团队管理，从数据架构到前端工程化，从故障应急到性能优化，从异步消息与事件驱动到分布式难题，从多租户 SaaS 到国际化部署，从 AI 组织转型到 AI 原生创业，从自进化组织到 AI 信任校准，从 Codebase 认知债到 Agent Harness 工程化，从 AI 致命三件套到 AI 安全纵深防御，从 AI 评测工程到 MCP/A2A 协议，从 AI 成本经济学到 AI 可观测性，从 RAG 到向量数据库与 Embedding，从 AI 合规到 AI 私有化部署，从 Prompt 工程到多模态 AI 应用 —— 一篇一个核心主题，篇篇独立又互相串联。
...
graph LR
  ...
  class B,H1,H2,H3,H4,H5,H6,H7,H8,H9,H10,H11,H12,H13,H14,H15,H16 purple
```
...
| 🟣 紫 | AI 时代 | 11 学徒、27 组织、28 创业、29 自进化、30 信任、31 认知债、32 Harness、33 致命、34a/34b 评测、35a/35b 协议、36a/36b Token、37 可观测、38 RAG、39 向量库、40 合规、41 私有化 |
...
> 以下按**叙事逻辑**排列（前传 → 续集一 → 正传 → 终章 → 番外 → 续集二 ~ 续集十七 → 番外七-八），而非文件编号顺序。每篇标题标注文件编号，方便检索。
>
> ⚠️ **2026 重要说明**：34/35/36 三篇已拆分为 34a/34b、35a/35b、36a/36b（结构性优化），旧文件保留为 30 行重定向指引。续集十三-十七（37-41）+ 番外七-八（42-43）为本期新增。
...
### 续集十：[AI 评测工程（基础篇）](./34a-ai-evaluation-fundamentals.md) + [流水线篇](./34b-ai-evaluation-pipeline.md)
...
### 续集十一：[MCP 协议（基础篇）](./35a-mcp-protocol.md) + [A2A 协议（进阶篇）](./35b-a2a-protocol.md)
...
### 续集十二：[AI 成本经济学（结构篇）](./36a-ai-token-cost-structure.md) + [优化篇](./36b-ai-token-cost-optimization.md)
...
### 续集十三：[AI 可观测性](./37-ai-observability.md)
...
### 续集十四：[RAG 检索增强生成](./38-rag-retrieval-augmented-generation.md)
...
### 续集十五：[向量数据库与 Embedding 实战](./39-vector-database-and-embedding.md)
...
### 续集十六：[AI 合规与监管](./40-ai-compliance-and-regulation.md)
...
### 续集十七：[AI 模型私有化部署](./41-ai-private-deployment.md)
...
### 番外七：[Prompt 工程深度专题](./42-prompt-engineering.md)
...
### 番外八：[多模态 AI 应用工程](./43-multimodal-ai.md)
...
- [术语表](./glossary.md) —— 220 词条速查，按 36 大主题分类
...
| AI 工程师 | 续集一 → 续集二 → 续集三 → 续集四 → 续集六 → 续集五 → 续集七 → 续集八 → 续集九 → 续集十 → 续集十一 → 续集十二 → 前传 → 正传 3 | Agent 全景 → 人机协作 → 组织转型 → AI 原生创业 → 幻觉护栏 → 自进化组织 → 认知债 → Harness → 致命三件套 → AI 评测 → 协议层 → AI 成本 → 架构基础 → 安全护栏 |
| 架构师 | 前传 → 续集一 → 续集三 → 续集四 → 续集五 → 续集七 → 续集八 → 续集九 → 续集十 → 续集十一 → 续集十二 → 正传 1/2/3/6/11/12 → 番外三 → 终章 | 从架构到 AI 到组织转型到 AI 原生创业到自进化到认知债到 Harness 到致命三件套到 AI 评测到协议层到 AI 成本到工程到分布式到 SaaS 到管理，全局视角 |
...
| CTO / VP | 终章 → 续集三 → 续集四 → 续集五 → 续集七 → 续集八 → 续集九 → 续集十 → 续集十一 → 续集十二 → 番外二 → 番外三 → 前传 → 续集一 | 组织管理 → AI 转型 → AI 原生创业 → 自进化组织 → 认知债 → Harness → 致命三件套 → AI 评测 → 协议层 → AI 成本 → 云成本 → SaaS 架构 → 架构 → AI |
...
续集十三：AI 客服答非所问 → 3 大传统支柱 + 4 大 AI 支柱 → 7 大支柱 → 5 大工具栈 → OpenLLMetry → L1-L5 成熟度
续集十四：AI 幻觉率 29% → RAG 5 环节 → 7 大模式（Hybrid/ReRank/GraphRAG）→ 4 大陷阱 → 幻觉率降至 3%
续集十五：向量库慢/召回差/成本高 → 6 大向量库 → Embedding 选型 → HNSW 调优 → Hybrid → 召回率 92% + 月成本 -70%
续集十六：AI 翻车被罚 200 万 → EU AI Act / 中国办法 → 4 大支柱 → 4 大体系 → 5 大风险 → 整改后 0 罚款 + 1000 万收益
续集十七：OpenAI 封号 + 数据不能出云 → 私有化 vs API → 5 大形态 → 4 大推理框架 → AWQ/INT4 → LoRA → 5 年 TCO 对比
番外七：AI 答非所问 + 输出不稳 + 成本失控 → 10 大 Prompt 模式 → 7 大高级技术 → 4 大优化 → 4 大模板库 → 准确率 +13% / 成本 -69%
番外八：AI 只会文字 → 5 大模态 → 6 大模型（GPT-4o/Claude/Gemini）→ 5 大场景 → 5 大挑战 → 用户覆盖 40% → 95% / 收入 +200%
