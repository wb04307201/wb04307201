...
### 番外八：[看世界的方式决定了你造什么系统](./31-modeling-evolution.md)

> 从"管对象"到"管动作"到"管组织"，看数字化认知的三次跃迁

三个架构师面试，同一个考题，三种完全不同的答案。OOD 看到实体，DDD 看到行为，TOGAF 看到组织。不是谁更高级，而是认知深度决定了方法论的选型。升维思考，降维执行。

**核心概念**：OOD → DDD → TOGAF · 康威定律 · 逆康威定律 · 业务能力地图 · 限界上下文 · 认知跃迁陷阱

**适合**：架构师 / 技术管理者 / 新晋 Tech Lead
...
| 架构师 | 前传 → 续集一 → 续集五 → 正传 1/2/3/6/11/12 → 番外三 → 番外八 → 终章 | 从架构到 AI 到自进化到工程到分布式到 SaaS 到认知跃迁到管理，全局视角 |
...
| CTO / VP | 终章 → 番外八 → 续集三 → 续集五 → 番外二 → 番外三 → 前传 → 续集一 | 组织管理 → 认知跃迁 → AI 转型 → 自进化组织 → 云成本 → SaaS 架构 → 架构 → AI |
...
番外八：三个架构师三种答案 → OOD看对象 → DDD看动作 → TOGAF看组织 → 升维思考降维执行
```
...
| 康威定律 | 终章 第1章 | 前传 第4章（垂直拆分）、番外八（认知跃迁阶段3）、续集一 第5章（Multi-Agent）|
| 逆康威定律 | 番外八 | 终章（组织架构调整）、前传（垂直拆分→组织拆分）|
...
| 限界上下文 / Bounded Context | 前传第八章 | 终章（康威定律与组织边界）、番外三（多租户上下文隔离）、番外八（认知跃迁阶段2→3）|
| 领域事件 / Domain Event | 前传第八章 | 正传 13（事件溯源）、正传 11（消息队列异步通信）|
| 微服务 / Microservices | 前传第八章 | 正传 6（API 设计）、正传 12（分布式难题）、终章（平台工程 IDP）|
| OOD → DDD → TOGAF | 番外八 | 前传第八章（DDD 落地）、终章（康威定律）、番外一（重构决策）|
| 认知跃迁 / Cognitive Evolution | 番外八 | 前传（架构演进的认知驱动力）、终章（从厨师到 CEO 的认知升级）|
| 升维思考降维执行 | 番外八 | 终章（战略→战术→执行）、番外六（标准化 vs 灵活性的权衡智慧）|
