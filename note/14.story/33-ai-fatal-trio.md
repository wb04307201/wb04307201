...
> **系列定位**：本篇是「阿明餐厅」系列的**续集九**。在[《食安大检查》](./06-security-architecture.md)中，我们讲过传统系统的六大防线（认证/权限/加密/零信任/审计/脱敏）。在[《AI 的"黑暗料理"》](./30-ai-hallucination-safety.md)中，我们讲过 AI 幻觉与信任校准。在[《Agent Harness》](./32-agent-harness.md)中，我们讲过 4 层 Guardrails。但 2026 年的真实事故告诉我们：AI 系统有 **3 个致命漏洞**，任何一个单独出现都是 P0 事故，**三个组合起来就是"一次攻击致命"**。
...
2. 工具权限分级
   - 工具按"风险等级"分级（详见[《Agent Harness》](./32-agent-harness.md)）
   - 高风险工具强制 HITL
   - 写操作必须有"二次确认"
...
在展开 4 层防护之前，先澄清一个常见混淆：本篇的 **"4 层防护"**（预防/检测/缓解/恢复）和[《AI 的"黑暗料理"》](./30-ai-hallucination-safety.md)第四章的 **"三层护栏"**（模型/系统/业务）**不是同一组概念**，互补而不重叠。
...
| **所属系列** | [续集六 · 30](./30-ai-hallucination-safety.md) | 续集九 · 33（本篇） |

**两者关系**：30 解决"AI 在没有攻击时是否可信"，33 解决"AI 在被攻击时是否扛得住"。**完整的 AI 安全 = 30 的护栏 + 33 的防护 + 32 的 Harness Guardrails**（详见[《Agent Harness》](./32-agent-harness.md)第六章），三者缺一不可。

读完本章后，建议回到[《AI 的"黑暗料理"》](./30-ai-hallucination-safety.md)第四章，对比阅读，理解"内部防线"与"外部防线"的协同。
...
Layer 1 - 预防层
  - 工具最小化（[《Agent Harness》](./32-agent-harness.md) 6.1）
  - 输入清洗（删除注入关键词）
  - System Prompt 加固
  - 训练数据去敏
...
- [AI 的"黑暗料理"](./30-ai-hallucination-safety.md) —— AI 幻觉与三件套的"输出过滤"层强相关
- [Codebase 认知债](./31-codebase-cognitive-debt.md) —— 认知债会放大三件套的风险（AI 看不懂的代码更容易被注入劫持）
- [Agent Harness](./32-agent-harness.md) —— Harness 的 4 层 Guardrails 是三件套防护的工程化实现
...
- [会自我进化的厨房](./29-self-evolving-company.md) —— 自进化组织的安全是"自动红队 + 自动修复"
