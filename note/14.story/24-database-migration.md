...
> **系列定位**：本篇是「阿明餐厅」系列的**正传 15**。在[前传](./02-system-architecture-evolution.md)中，阿明的数据库从单机 MySQL 演进到读写分离再到分库分表。但"架构怎么演进"只讲了一半 —— 另一半是**数据怎么搬家？** 一个日活 10 万的系统，不可能停服 8 小时慢慢迁移。
...
- [传菜窗口的智慧](./17-async-messaging.md) —— 双写期间的增量同步可以借助消息队列，保证最终一致性
