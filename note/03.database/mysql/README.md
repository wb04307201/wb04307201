...
### 当前读和快照读
MySQL中的当前读（Current Read）和快照读（Snapshot Read）是两种重要的读取方式，它们在处理并发事务中的数据一致性和隔离性问题时起着关键作用。
...
##### 特点
非阻塞读取：因为读取的是历史版本，不会等待其他事务释放锁，提高了并发读的效率。
事务隔离性：通过读取事务开始时的数据版本，可以避免脏读、不可重复读等问题，但默认情况下可能还会有幻读的问题，这取决于数据库的隔离级别设置。
内存和磁盘开销：为了维持多个数据版本，系统需要额外的存储空间，并且在垃圾回收旧版本数据时会有一定的管理开销。

##### 场景
单纯的 SELECT 语句，但不包括这两类 SELECT 语句
```sql
SELECT ... FOR UPDATE
...
#### 当前读 （一致性锁定读）
##### 定义
当前读是指读取数据时获取的是数据的最新版本，同时可能会锁定读取的数据行，以确保数据的一致性，避免其他事务并发修改。

##### 特点
- **最新数据**：读取的数据是当前最新的已提交版本。
- **锁机制**：会对读取的数据行加锁，以保证其他并发事务不会修改当前记录。

##### 场景
给行记录加 X 锁或 S 锁，当前读的一些常见 SQL 语句类型
```sql
# 对读的记录加一个X锁
SELECT...FOR UPDATE
# 对读的记录加一个S锁
