...
创建型模式解决**"由谁、怎么、什么时候"实例化对象**这一基础问题。难点不在"用一个 new 还是工厂"，而在于**让实例化逻辑可控 + 可扩展 + 与业务解耦**：工厂方法把创建延迟到子类，单例保证全局唯一，原型用克隆代替构造，建造者分离复杂对象的构造与表示。本篇围绕"实例化权归属"这条主线，对比工厂方法 vs 抽象工厂、单例 vs 静态工厂、原型 vs 建造者的取舍。
