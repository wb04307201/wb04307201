...
> **定位**：GoF 23 种设计模式的 Java 实现与选型指南 —— 笔记已按 GoF 分类拆分为 3 个子文件。 的核心原理、实现与最佳实践。

> GoF 23 种设计模式的 Java 实现与选型指南 —— 笔记已按 GoF 分类拆分为 3 个子文件。

---
## 引言：23 种 GoF 设计模式的真正难点

23 种 GoF 设计模式不是用来背诵的——真正的难点是**同一问题多种模式都能解，何时该让步、何时该坚持**。本篇围绕"创建型如何控实例化、结构型如何组对象、行为型如何分职责"三条主线，把抽象工厂 vs 工厂方法、装饰器 vs 代理、策略 vs 状态等高频二选一的取舍讲清楚。
