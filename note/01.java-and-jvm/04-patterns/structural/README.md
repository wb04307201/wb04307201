...
结构型模式关心**类和对象如何组合成更大的结构**——通过继承还是通过组合、是否暴露实现、是否在运行时改变结构。难点不在选 Adapter 还是 Bridge，而在**区分"接口兼容"（Adapter）、"抽象与实现解耦"（Bridge）、"动态给对象加职责"（Decorator）vs "用代理控制访问"（Proxy）**。本篇围绕"组合方式 + 暴露程度"两条轴，对比 Adapter/Bridge、Decorator/Proxy、Facade/Mediator 等高频二选一。
