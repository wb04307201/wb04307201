...
> **系列定位**：本篇是「阿明餐厅」系列的**番外六**。在[前传](./02-system-architecture-evolution.md)中，阿明从一家小面馆做到了全国连锁。现在，他要把餐厅开到海外 —— 第一站：东京。但"出海"不只是翻译菜单那么简单：**时区、货币、法规、网络延迟、数据合规**，每一个都是全新的挑战。
...
- [传菜窗口的智慧](./19-realtime-eventdriven.md) —— 异步消息，跨区域数据同步的核心技术就是异步消息复制
- [十家店的烦恼](./17-distributed-puzzles.md) —— 分布式系统的经典难题，多区域部署就是更大规模的分布式
- [阿明的加盟帝国](./18-saas-multitenant.md) —— SaaS 多租户架构，出海时不同国家的租户隔离策略不同
- [厨房实况直播](./19-realtime-eventdriven.md) —— 实时与事件驱动架构，跨区域的事件传播延迟是核心挑战
- [一个厨房，四个门面](./20-multiplatform-architecture.md) —— 多端架构，不同国家的用户偏好不同的客户端（LINE / WhatsApp / 微信）
- [懂你的菜单](./21-search-recommendation.md) —— 搜索与推荐，推荐算法需要适应不同国家的饮食文化差异
- [菜谱标准化之路](./07-from-chef-to-ceo.md) —— 技术文档与知识管理，出海文档需要多语言版本
- [仓库搬家不停业](./22-database-migration.md) —— 数据库迁移，多区域部署可能涉及跨区域的数据迁移
- [预制菜还是现炒](./23-lowcode-platform.md) —— 低代码平台，多语言的低代码编辑器本身也需要国际化
- [厨房大换岗](./25-ai-org-transformation.md) —— AI 组织转型的全球化考量，不同文化背景下 AI 转型路径不同
- [阿明的二次创业](./26-ai-native-startup.md) —— AI 原生创业的全球化市场，AI 产品如何适配不同地区的用户
- [会自我进化的厨房](./27-self-evolving-company.md) —— Agent Loop 的多区域部署，Agent 在不同时区和文化中自我进化
- [AI 的"黑暗料理"](./28-ai-hallucination-safety.md) —— AI 幻觉的多语言多文化差异，不同语言环境下的幻觉模式和治理策略
...
- [21-multiplatform-architecture.md](./20-multiplatform-architecture.md) —— 正传 13，国际化与多端架构的协同：不同地区的 UI 适配
- [22-search-recommendation.md](./21-search-recommendation.md) —— 番外四，国际化搜索推荐：跨语言的查询理解与本地化推荐
...
<!-- TODO: 拆分候选 (739 行 / 11 个 H2，超 500+8 阈值） -->
