...
> **系列定位**：本篇是「阿明餐厅」系列的**续集十一（上）**。在[续集一 · 《当餐厅长出大脑》](./01-ai-agent-architecture.md)第五章，我们讲了多智能体协同（Orchestrator + 消息总线）。在[续集八 · 《Agent Harness》](./30-agent-harness.md)第三章，我们讲了 Tool 设计。但都还是"自己设计、自己实现"。2024-2026 年，业界出现了**两个重量级协议标准** —— **MCP（Model Context Protocol，Anthropic 主导）** 和 **A2A（Agent-to-Agent，Google 主导）** —— 它们正在成为 AI 时代的"TCP/IP"，让 Agent 与 Agent、Agent 与工具之间有了**统一的"语言"**。本篇（上）专门讲 **MCP —— Agent ↔ 工具的"USB-C"**；下篇[35b](./33b-a2a-protocol.md)专门讲 **A2A + 协议治理**。

> **兄弟篇**：35b · A2A 协议 + 协议治理（[← 点击阅读](./33b-a2a-protocol.md)）

---

## 引言：20 个 Agent 互相听不懂对方说话

> **阿明的厨房类比（开篇场景）**：阿明的 80 家分店有 20 个 AI 智能设备（智能灶、自动翻炒机、智能点餐机、智能收银机、智能清洁机器人……）。每个设备都有自己的"插头规格"和"操作语言" —— 智能灶只认 220V / 三孔，清洁机器人只认 USB-C。**20 个设备 × 5 种插头 = 100 种转换器**！阿明需要"万能 USB-C"协议 —— 所有设备统一一个接口。本章讲 MCP 协议（万能插口）如何让 Agent 时代告别"插头混乱"。
...
**本篇（上）** 聚焦 **MCP** —— Agent 调用工具的"USB-C"。**下篇[35b](./33b-a2a-protocol.md)** 聚焦 **A2A + 协议治理**（安全 / 可观测 / 趋势）。

---

> **阿明的厨房类比（第一章）**：MCP 就像厨房设备的"USB-C 统一接口"。以前每个设备（智能灶/清洁机器人/收银机）都有自己的 API（HTTP REST / GraphQL / gRPC），要写 100 种适配代码。MCP 统一规定 —— 所有设备只要插上 USB-C，AI 厨师就能用。本章讲 MCP 的"万能插口"架构。
...
→ **A2A 协议** 解决这些问题（见[35b 第二章](./33b-a2a-protocol.md#第二章a2a-是什么--agent-协同的邮件协议)）
...
MCP Server 拿到的是 Agent 的"全部上下文"
  → 任何漏洞 = 全部泄露
  → 这是 [33 致命三件套](./31-ai-fatal-trio.md) 中"数据外泄"的新攻击面
```
...
> **阿明的厨房类比（第二章）**：阿明面对 3 种选型 —— 统一 USB-C 接口（MCP，让 AI 调用工具）、统一对讲机（A2A，让 Agent 之间沟通）、还是自己造协议（自研）。三种方案各适合什么场景？本章阿明用"厨房场景"做类比，帮你选对方案。
...
> **阿明的厨房类比（第三章）**：好协议就像好 USB-C —— 在哪都能用。本章阿明用 6 大设计原则解读：能力可发现（设备说明书）、安全沙箱（防误操作）、流式响应（边炒边上菜）、错误恢复（断电保护）……
...
> **阿明的厨房类比（第四章）**：阿明要给 80 家分店的 1000 个厨房设备装 USB-C 接口 —— 智能灶、调味料罐、清洁机器人、收银机、点餐机。每个设备都得"MCP 化"。本章阿明分享 5 步落地法 + 80 家分店批量部署经验。
...
# 防止 [33 致命三件套](./31-ai-fatal-trio.md) 的"过度授权"
```
...
下一步阅读[35b · A2A 协议 + 协议治理](./33b-a2a-protocol.md)—— 工具层有 MCP，协同层用什么？安全 / 可观测 / 趋势怎么看？

---

## 延伸阅读

- [35b · A2A 协议 + 协议治理（下篇）](./33b-a2a-protocol.md) —— A2A 协议 + 5 大安全陷阱 + 可观测性 + 5 大趋势
- [当餐厅长出大脑](./01-ai-agent-architecture.md) —— 续集一，AI Agent 7 大模块，第五章多智能体协同是本篇前传
- [Agent Harness](./30-agent-harness.md) —— 续集八，Harness 内的 Tool 设计直接对接 MCP 协议
- [AI 致命三件套](./31-ai-fatal-trio.md) —— 续集九，协议层是三件套的"新攻击面"
- [厨房装监控](./05-observability.md) —— 正传 2，协议层的可观测性与传统 observability 同构
- [AI 评测工程（基础篇）](./32a-ai-evaluation-fundamentals.md) / [（流水线篇）](./32b-ai-evaluation-pipeline.md) —— 续集十，协议层是评测的对象之一
- [从厨师到 CEO](./07-from-chef-to-ceo.md) —— 终章，协议治理是平台工程 IDP 的核心
- [会自我进化的厨房](./27-self-evolving-company.md) —— 续集五，Agent 协议让自进化组织的"自循环"成为可能
- [Codebase 认知债](./29-codebase-cognitive-debt.md) —— 续集七，协议文档化能降低认知债
...
> 好的工具集成，不是"给 AI 装一堆插件"，而是"**用一套标准协议，让 AI 像人一样自然地使用工具**"。

← [返回系列导读](./index.md) | [下篇：35b A2A + 治理 →](./33b-a2a-protocol.md)
<!-- TODO: 拆分候选 (698 行 / 9 个 H2，超 500+8 阈值） -->
