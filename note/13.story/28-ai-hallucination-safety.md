...
> **系列定位**：本篇是「阿明餐厅」系列的**续集六**。在前几篇续集中，阿明学会了 AI 的组织转型（[续集三](./25-ai-org-transformation.md)）、AI 原生创业（[续集四](./26-ai-native-startup.md)）和自进化组织（[续集五](./27-self-evolving-company.md)）；本篇聚焦一个贯穿所有 AI 应用的核心问题 —— 当 AI "一本正经地胡说八道"时，你怎么办？
...
> **与本篇的"三层护栏"互补的概念**：本篇的"三层护栏"（模型/系统/业务）解决的是**AI 内部出错**（幻觉、误判）的检测；而[续集九 · 33 · 《AI 致命三件套》](./31-ai-fatal-trio.md)的"4 层防护"（预防/检测/缓解/恢复）解决的是**AI 被外部攻击**（注入、越权、泄露）的防御。两者**互补不重叠**，完整的 AI 安全需要两者叠加。详见[33 第六章 6.0 节对比表](./31-ai-fatal-trio.md)。
...
这和[续集五](./27-self-evolving-company.md)中"自进化组织"的理念完全一致 —— AI 的安全不是静态的防线，而是一个不断进化的免疫系统。
...
- [十家店的烦恼](./17-distributed-puzzles.md) —— 多门店场景下 AI 安全策略的一致性：不同门店能否使用不同的审核标准
- [阿明的加盟帝国](./18-saas-multitenant.md) —— 多租户场景下 AI 安全策略的隔离与共享：总部策略 vs 门店自定义
- [厨房实况直播](./19-realtime-eventdriven.md) —— AI 输出的实时监控和告警，事件驱动架构的用武之地
- [一个厨房，四个门面](./20-multiplatform-architecture.md) —— 不同渠道（堂食、外卖、小程序）的 AI 安全策略可能需要差异化配置
- [懂你的菜单](./21-search-recommendation.md) —— AI 推荐系统本身就存在幻觉风险，推荐算法的可靠性是安全的基础
- [菜谱标准化之路](./07-from-chef-to-ceo.md) —— 食材兼容性数据库和知识库的建设，是知识工程在安全领域的应用
- [仓库搬家不停业](./22-database-migration.md) —— AI 系统的知识库更新和模型升级，如何做到在线不停服
- [预制菜还是现炒](./23-lowcode-platform.md) —— AI 审核工作流的低代码配置：让非技术人员也能调整审核规则
- [阿明出海记](./24-globalization.md) —— AI 幻觉在不同文化语境下的差异：食材相克的认知在不同国家可能截然不同
- [厨房大换岗](./25-ai-org-transformation.md) —— 续集三，AI 组织转型中的岗位重设计：审核员这个新角色如何产生
- [阿明的二次创业](./26-ai-native-startup.md) —— 续集四，AI 原生创业中如何从第一天就设计好安全护栏
- [会自我进化的厨房](./27-self-evolving-company.md) —— 续集五，自进化组织的反馈闭环是本文落地框架的进化引擎
...
- [22-search-recommendation.md](./21-search-recommendation.md) —— 番外四，搜索推荐系统的"幻觉"：推荐不相关内容也是一种幻觉
- [32-agent-harness.md](./30-agent-harness.md) —— 续集八，Agent Harness 的幻觉防护：上下文注入、输出校验、人类反馈
...
<!-- TODO: 拆分候选 (557 行 / 11 个 H2，超 500+8 阈值） -->
