...
> **系列定位**：本篇是「阿明餐厅」系列的**续集十（下）**。本篇接续[34a · AI 评测基础（上）](./32a-ai-evaluation-fundamentals.md)，**聚焦"评测流水线与生产化"**：5 层流水线架构、RAG 专项评测、红队测试、在线 A/B、平台工程化。本篇与上篇是一对孪生篇 —— 上篇讲"评什么、怎么评"，下篇讲"怎么跑、怎么用、怎么工程化"。

> **兄弟篇**：34a · AI 评测基础（[← 点击阅读](./32a-ai-evaluation-fundamentals.md)）

---

## 引言：从"评测基础"到"流水线生产化"

> **阿明的厨房类比（开篇场景）**：上篇（34a）阿明学会了"试菜标准"—— 给 AI 厨师打分。但"试一次"不够 —— 上线后还要"持续试"。本章阿明把"试菜间"升级成"流水线质检" —— 每道新菜上线前自动试菜、每月试菜一次、每年试菜升级。

[34a · 评测基础（上）](./32a-ai-evaluation-fundamentals.md)讲的是"评什么、怎么评" —— 4 大挑战、6 大维度、5 大黄金集心法、LLM-as-Judge。
...
> **阿明的厨房类比（第五章）**：传统试菜是"师傅一筷子"，AI 时代要做"自动化试菜产线" —— AI 出了一道菜，自动进入试菜流水线（5 关），不通过就不上菜单。本章阿明学会把"试菜间"工程化。

## 第五章：Eval 流水线架构 —— 从试菜到流水线质检，评测也要工程化
...
> **阿明的厨房类比（第六章）**：RAG 是"AI 厨师看菜谱做菜" —— 评测 RAG 要专门测"菜谱翻得对不对"（检索）、"做的菜对不对"（生成）、"用的菜谱新鲜吗"（数据更新）。本章阿明给 RAG 单独装了一套"菜谱质检系统"。

## 第六章：RAG 系统的专项评测 —— 翻菜谱翻得对不对，专门查这项
...
> **阿明的厨房类比（第七章）**：阿明雇了一群"美食评论家"专门来挑刺 —— 什么菜最难吃？什么搭配最怪？什么食材最容易引起过敏？本章讲"红队测试" —— 用恶意输入攻击 AI 系统，看能不能扛得住。

## 第七章：红队测试与对抗评测 —— 请人来砸场子，看防线扛不扛得住
...
> **阿明的厨房类比（第八章）**：阿明做 A/B 测试 —— 把"老厨师"和"AI 厨师"做的同一道菜同时上菜单，看哪道点单率高。本章阿明学会"在线 A/B" + "实时监控"双管齐下。

## 第八章：在线 A/B 测试与监控 —— 偷偷换菜谱，看客人买不买账
...
> **阿明的厨房类比（第九章）**：试菜数据多了，阿明建了一个"评测平台" —— 像中央厨房的质检产线，所有分店的试菜数据汇总、自动分析、生成报告。本章阿明把"试菜间"做成"自动化质检产线"。

## 第九章：评测平台的工程化实践 —— 把厨房质检变成一套自动化产线
...
- [34a · AI 评测基础（上篇）](./32a-ai-evaluation-fundamentals.md) —— 挑战 + 维度 + 黄金集 + LLM-as-Judge
...
- [AI 的"黑暗料理"](./28-ai-hallucination-safety.md) —— 续集六，AI 幻觉与本篇"忠实性"维度
- [AI 致命三件套](./31-ai-fatal-trio.md) —— 续集九，安全性评测与红队的"攻击面"重叠
- [Agent Harness](./30-agent-harness.md) —— 续集八，Harness 内的 Eval 流水线与本篇"评测平台"是嵌套关系
- [Codebase 认知债](./29-codebase-cognitive-debt.md) —— 续集七，认知债会放大评测难度
- [学徒的困境](./11-ai-learning-paradox.md) —— 续集二，AI 时代的人机协作，评测工作的角色变化
- [会自我进化的厨房](./27-self-evolving-company.md) —— 续集五，评测是自进化组织的"质量门"
- [从厨师到 CEO](./07-from-chef-to-ceo.md) —— 终章，评测平台是 IDP（内部开发者平台）的核心组件
- [差评危机](./15-incident-response.md) —— 正传 9，事故复盘 → 黄金集更新的工作流
- [38 · RAG 检索增强生成专题](./36-rag-retrieval-augmented-generation.md) —— 38 续集十四，本篇 6 章的 RAG 化详解
...
← [返回系列导读](./index.md) | [上篇：34a 评测基础 →](./32a-ai-evaluation-fundamentals.md)
<!-- TODO: 拆分候选 (738 行 / 10 个 H2，超 500+8 阈值） -->
