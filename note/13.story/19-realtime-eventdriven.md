...
> Saga 协调器的分布式事务实现细节详见[《十家店的烦恼》](./17-distributed-puzzles.md)，本篇关注事件流本身的设计。
...
> CDC 在数据迁移中的应用详见[《仓库搬家不停业》](./22-database-migration.md)，本篇关注其在事件驱动中的角色。
...
- [十家店的烦恼](./17-distributed-puzzles.md) —— Saga 协调器的分布式事务实现细节
- [阿明的加盟帝国](./18-saas-multitenant.md) —— 多租户资源隔离需要消息队列做租户级别限流
- [一个厨房，四个门面](./20-multiplatform-architecture.md) —— BFF 层通过消息队列与后端服务异步通信
- [懂你的菜单](./21-search-recommendation.md) —— 用户行为日志通过 Kafka 实时采集
- [菜谱标准化之路](./07-from-chef-to-ceo.md) —— 消息队列的架构决策应记录为 ADR
- [仓库搬家不停业](./22-database-migration.md) —— CDC 在数据迁移中的应用
- [厨房大换岗](./25-ai-org-transformation.md) —— AI 转型中的消息传递模式变化
- [阿明的二次创业](./26-ai-native-startup.md) —— AI 原生创业中的异步架构选择
...
- [22-search-recommendation.md](./21-search-recommendation.md) —— 番外四，搜索推荐的实时性依赖事件驱动架构：用户行为事件触发索引更新
...
<!-- TODO: 拆分候选 (758 行 / 10 个 H2，超 500+8 阈值） -->
