...
- [传菜窗口的智慧](./19-realtime-eventdriven.md) —— 异步消息架构，多端之间的消息同步和事件驱动设计
- [十家店的烦恼](./17-distributed-puzzles.md) —— 分布式系统的经典难题，多端库存同步中的分布式一致性问题
- [阿明的加盟帝国](./18-saas-multitenant.md) —— 多租户 SaaS 架构，加盟店的多端管理（每个加盟商有自己的端和配置）
- [厨房实况直播](./19-realtime-eventdriven.md) —— 实时事件驱动架构，骑手 App 的实时位置推送和多端状态同步
- [懂你的菜单](./21-search-recommendation.md) —— 搜索与推荐系统，多端场景下推荐结果需要按端的展示能力做适配
- [菜谱标准化之路](./07-from-chef-to-ceo.md) —— 技术文档与知识管理，多端开发团队的文档协作和知识共享
- [仓库搬家不停业](./22-database-migration.md) —— 数据库迁移实战，多端共享数据库的迁移策略和零停机方案
- [预制菜还是现炒](./23-lowcode-platform.md) —— 低代码平台选型，多端场景下低代码能否替代定制化开发
- [阿明出海记](./24-globalization.md) —— 国际化与全球化，多端架构在多语言多时区多币种场景下的挑战
- [厨房大换岗](./25-ai-org-transformation.md) —— AI 转型对多端的影响，不同端的人机协同模式不同
- [阿明的二次创业](./26-ai-native-startup.md) —— AI 原生创业的多端覆盖策略，AI 辅助快速适配多个平台
- [会自我进化的厨房](./27-self-evolving-company.md) —— Agent Loop 的跨端协作，Agent 需要统一处理多端输入输出
- [AI 的"黑暗料理"](./28-ai-hallucination-safety.md) —— AI 幻觉在不同端的表现，多端场景的幻觉检测和展示策略差异化
- [09.front-end / 08 跨端](../05.frontend/08-cross-platform/README.md) —— 移动 / 桌面 / 小程序 / PWA：跨端形态全景与选型
- [09.front-end / 05 架构 / BFF](../05.frontend/05-architecture/bff/README.md) —— BFF 模式详细实践：本章 BFF 概念的工程化落地
- [09.front-end / 09 前端与 AI](../05.frontend/09-frontend-and-ai/README.md) —— AI Native UI / AI SDK / Vibe Coding
...
- [19-saas-multitenant.md](./18-saas-multitenant.md) —— 番外三，多端架构支持 SaaS 多租户的入口分流：不同终端对应不同租户
- [26-globalization.md](./24-globalization.md) —— 番外六，多端架构下的国际化：不同地区、不同语言、不同端 UI 适配
...
<!-- TODO: 拆分候选 (604 行 / 11 个 H2，超 500+8 阈值） -->
