...
> **系列定位**：本篇是「阿明餐厅」系列的**续集十一（下）**。本篇接续[35a · MCP 协议（上）](./33a-mcp-protocol.md)。上篇讲 MCP —— Agent 调用工具的"USB-C"；本篇讲 **A2A —— Agent 协同的"邮件协议"**，以及**协议层的治理**（安全 / 可观测 / 趋势）。本篇与上篇是一对孪生篇 —— 上篇讲"工具层"，下篇讲"协同层 + 治理层"。

> **兄弟篇**：35a · MCP 协议（[← 点击阅读](./33a-mcp-protocol.md)）

---

## 引言：从 MCP 到 A2A + 治理

> **阿明的厨房类比（开篇场景）**：上篇（35a）讲了"USB-C 统一插口"（MCP）—— 解决了 AI 调用工具的复杂度。但阿明还有第二个问题：80 家分店的 20 个 AI 智能设备（智能灶/清洁机器人/点餐机/收银机），**设备之间怎么协作**？智能灶做完菜，要通知清洁机器人来收碗 —— 这就需要"对讲机协议"（A2A）。本章阿明装"厨房对讲机系统"。

---

> **阿明的厨房类比（第五章）**：MCP 让 20 个智能设备有了"USB-C 插口"，但要让它们"互相沟通"，还得用 A2A（Agent-to-Agent）。就像厨房里所有设备都通电后，还要"对讲机系统" —— 智能灶做完喊一声"上菜了"，点餐机听到后喊一声"通知顾客"，清洁机器人听到后喊一声"准备收碗"。这就是 A2A —— **Agent 时代的"邮件协议"**。
...
> **阿明的厨房类比（第六章）**：80 家分店装了 20 个 AI 智能设备，怎么让它们协同做一道"满汉全席"？每台设备都有自己的 Agent Card（功能说明书）—— 智能灶能做川菜、点餐机接单、清洁机器人洗碗。本章阿明用"厨房总指挥"（Orchestrator）+ "对讲机"（A2A）实现"端到端自动化厨房"。
...
> **阿明的厨房类比（第七章）**：对讲机虽好，但也给"小偷"留了后门 —— 伪装成智能灶的假设备（Agent Card 伪造）、跨店信任滥用（恶意接单）、对讲机注入（恶意指令）、清洁机器人被劫持（产物污染）、协同放大（小偷利用 20 个设备的协同）。本章阿明给厨房对讲机系统装"5 道防盗门"。

## 第七章：协议层的 5 大安全陷阱 —— 万能插口虽好，也给小偷留了后门

协议层给了我们便利，**也给了我们新的攻击面**。阿明结合[33 致命三件套](./31-ai-fatal-trio.md)总结了 5 大陷阱。
...
> **阿明的厨房类比（第八章）**：阿明的厨房出了事故："上周五晚上 7 点，5 号桌的菜为什么迟了 10 分钟？"老陈调出对讲机日志 —— 智能灶 7:05 完成，点餐机 7:06 通知顾客，清洁机器人 7:10 才来收碗…… 整个流程一清二楚。本章讲 A2A 的"厨房黑匣子"。
...
> **阿明的厨房类比（第九章）**：USB-C 已经一统天下，但厨房设备的协议还在进化 —— 未来可能出现"无线充电 + 自动识别食材 + 智能烹饪"的一体化协议。本章阿明展望协议层的下一个 5 年。
...
- [35a · MCP 协议（上篇）](./33a-mcp-protocol.md) —— MCP 诞生 + 架构 + 选型 + 6 大原则 + 5 场景落地
...
> 好的多 Agent 协作，不是"让 AI 自己开会"，而是"**每个 Agent 各司其职，通过协议高效交接，像一个训练有素的厨房团队**"。

← [返回系列导读](./index.md) | [上篇：35a MCP →](./33a-mcp-protocol.md)
<!-- TODO: 拆分候选 (689 行 / 10 个 H2，超 500+8 阈值） -->
