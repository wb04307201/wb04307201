<!--module:
  ...
  slug: 08.application-systems/02-production/mes
  type: article
  category: 主模块子文章
...
- **MESA 11 大功能模块**：资源管理 / 工序调度 / 生产分配 / 文档管理 / 数据采集 / 运维 / 质量管理 / 追溯 / 监控 / 绩效分析 / 通用
- **三层架构**：ERP 计划层 → MES 执行层 → SCADA/PLC 控制层
- **典型数据量级**：500GB-50TB / 活跃用户 200-5000 / 日均增量 GB 级
- **核心能力**：8 类（生产订单 / 车间调度 / 数据采集 / 质量管理 / 物料追溯 / 设备集成 / SOP 电子化 / 绩效分析）
- **典型行业**：5 类（离散制造 / 流程制造 / 半导体 / 医疗器械 / 航空航天）
- **集成架构**：5 类（MES-ERP / MES-PLM / MES-SCADA / MES-WMS / MES-QMS）
- **选型维度**：6 类（业务规模 / 行业 Know-how / 集成能力 / 实时性能 / 数据采集 / 实施能力）
- **关键考量**：5 项（数据采集 / 主数据治理 / 实施路径 / 变更管理 / 长期演进）
- **常见陷阱**：6 类（流程未梳理 / 主数据脏 / 数据采集难 / 车间抵触 / 期望值过高 / 升级失当）
- **国产化替代**：盘古 / 摩尔元山 / 黑湖智造 / 鼎捷
- **所属价值链**：02 生产制造
