...
## 二、模块 README 模板（8-section index template）
...
> **现状盘点（2026-07-08 校对）**：仓库内有 **40 个 PNG 文件**（其中 34 个为教程 UI 截图，6 个为架构/流程图），仅 **7 篇文档** 实际嵌入 **37 处 PNG**（另有 3 篇 .md 在正文/链接中提及 `.png` 但无嵌入）。历史遗留的无意义命名 PNG 已清理完毕。
>
...
> 📌 **H2 数字编号规范**（2026-07-29 澄清）：H2 及更低级别标题**允许**使用阿拉伯数字编号（如 `## 0. 问题背景`、`## 1. 核心原理`、`## 2. 解决方案`），这是全库事实标准（1400+ 处使用）。仅 H1 禁止数字编号。
...
## 十、frontmatter 规范（已全量落地 781 个 README，find 校对 2026-07-25）
...
## 十一、模块 README 自检表（2026-07-01 务实版）
