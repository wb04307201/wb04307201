...
module:
  ...
  slug: application-systems/04-sales-service/crm
  type: article
  category: 主模块子文章
...
- **协同型 CRM（SCRM）**：社交化客户管理（微信生态/小红书/抖音），见 [SCRM 简讲](../README.md)
...
- **核心能力**：7 大模块（客户主数据 / SFA / 市场自动化 / 客服服务 / 客户成功 / 数据分析 / 流程自动化）
- **CRM 成熟度分级**：5 级（L1 基础 / L2 标准 / L3 高级 / L4 卓越 / L5 生态）
- **销售漏斗**：L→O→Q→C→O 五段式（Lead / Opportunity / Quote / Contract / Order）
- **客户视图 360°**：6 维度（基础档案 / 交易历史 / 交互历史 / 服务历史 / 社交行为 / 画像标签）
- **典型场景**：6 类（B2B 大客户 / B2C 零售 / SaaS 订阅 / 制造业经销商 / 金融代理人 / 渠道伙伴）
- **客户运营系统边界**：4 系统（CRM / SCRM / CDP / MA）
- **集成要点**：5 类（CRM-ERP / CRM-OMS / CRM-SCRM / CRM-CDP / CRM-客服）
- **集成模式**：4 类（紧耦合 / 松耦合 / 事件驱动 / iPaaS）
- **关键考量**：6 维度（流程标准化 / 数据质量 / 移动端 / SFA-BI 边界 / 国产 vs 国际 / 续费率）
- **销售流程陷阱**：3 类（先僵化再优化 / MDM 先行 / 60/40 法则）
- **CRM-BI 边界陷阱**：CRM 聚焦交易型报表 / BI 聚焦分析型报表
- **国产 vs 国际**：跨国/上市 → Salesforce/Dynamics；国内 → 销售易/纷享销客
- **选型自检维度**：5 项（流程固化 / 移动端 / 开放 API / 数据所有权 / 行业案例）
- **决策树**：5 层（业务模式 → 部署偏好 → 规模预算 → 集成生态 → TCO）
- **RFP 评分项**：5 大类 30+ 项（功能 30% / 性能 15% / 集成 25% / 合规 15% / 服务 15%）
- **POC 关键场景**：3 个（销售漏斗全程 / 客户 360° / 移动端外勤）
- **TCO 估算**（500 人销售）：License 30% / 实施 40% / 运维 20% / 升级迁移 10%
- **典型陷阱**：7 类（销售流程未标准化 / 销售抵触录入 / 客户主数据脏 / 报表依赖 IT / 续费涨价 / SCRM-CRM 不互通 / 私有化 ≠ 安全）
- **陷阱统计**：CRM 项目失败率 40-60%（30% 流程 / 25% 推广 / 20% 数据 / 15% 能力 / 10% 选型）
- **规避核心**：销售流程标准化 + MDM 数据治理先行 + 高层推动 + 移动端体验达标 + 自助 BI 解耦
- **所属价值链**：04 销售服务
