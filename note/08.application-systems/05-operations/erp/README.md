...
module:
  ...
  slug: application-systems/05-operations/erp
  type: article
  category: 主模块子文章
...
- 返回 [05 运营管理](../README.md) 章节

← [返回业务系统总览](../README.md)

## 📊 本节统计

- **核心模块**：7 大模块（财务 FI/CO / 采购 MM-PUR / 库存 MM-IM / 销售 SD / 计划 PP-PLAN / 生产执行 PP-PI / 人力资源 HR）
- **典型数据量级**：500GB-50TB / 活跃用户百人-万人 / 日均单据 10 万-100 万 / 年度凭证 百万-千万级
- **ERP 不擅长**：客户体验（CRM）/ 车间执行（MES）/ 数据可视化（BI）
- **核心边界**：4 类（vs CRM / vs MES / vs WMS / vs PLM / vs 财务）
- **主流厂商**：6 大类（国际大厂 / 国际二线 / 国内大型 / 国内中型 / 国内小型 / 开源）
- **典型场景**：6 类（大型集团 / 多组织 / 跨国 / 项目型 / 流程制造 / 离散制造）
- **集成架构**：5 上下游（CRM / PLM / MES / WMS / SRM-SCM）+ 横向 HR-财务-BI
- **选型维度**：6 类（业务规模 / 行业匹配度 / 实施能力 / 集成生态 / 总拥有成本 / 长期演进）
- **关键考量**：5 项（行业 Know-how / 实施周期 1-3 年 / 数据迁移占成本 30-40% / 主数据治理 / 主数据治理 80%）
- **国产化替代**：4 层级（ERP / MES / 数据库 / 中间件）
- **常见陷阱**：6 类（流程未梳理 / 主数据脏 / 期望值过高 / 项目治理弱 / 培训不足 / 升级失当）
- **所属价值链**：05 运营管理
- 关联系统：CRM（[CRM 深读](../../04-sales-service/crm/README.md)）/ PLM（[PLM 深读](../../01-rd-innovation/plm/README.md)）/ PDM（[PDM 深读](../../01-rd-innovation/pdm/README.md)）/ MES（[MES 深读](../../02-production/mes/README.md)）/ 财务模块（[05 运营管理章节](../README.md)）/ SCM（[03 供应链章节](../../03-supply-chain/README.md)）
