...
module:
  ...
  slug: application-systems/01-rd-innovation/plm
  type: article
  category: 主模块子文章
...
- **生命周期阶段**：5-6 阶段（概念 / 设计 / 工艺 / 生产 / 销售 / 退役）
- **核心能力**：8 类（BOM 中央仓库 / 工程变更 / 项目管理 / CAD 集成 / 文档管理 / 协作 / 流程审批 / 数据分析）
- **典型场景**：4 类（汽车新车型 / 电子产品多代演进 / 工程变更追溯 / 跨地域研发协同）
- **上下游集成**：4 类（PLM-CRM / PLM-ERP / PLM-MES / PLM-PDM）
- **PLM vs PDM vs ERP**：3 系统边界
- **主流厂商**：4 类（国际大厂 Siemens / Dassault / PTC / 国产 数码大方 / 华天）
- **选型维度**：6 类（业务规模 / 行业 / CAD 集成 / BOM 复杂度 / 实施能力 / TCO）
- **关键考量**：5 项（BOM 中央化 / 工程变更 / CAD 集成 / 主数据治理 / 长期演进）
- **常见陷阱**：5 类（流程未梳理 / BOM 与 ERP 不一致 / CAD 集成弱 / 实施超期 / 用户抵触）
- **所属价值链**：01 研发创新
