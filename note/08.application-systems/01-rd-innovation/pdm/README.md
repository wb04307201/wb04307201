...
module:
  ...
  slug: application-systems/01-rd-innovation/pdm
  type: article
  category: 主模块子文章
  summary: PDM（Product Data Management 产品数据管理） 本应该很简单，一句话定位：PLM 的核心子集，专注于产品数据本身（文档、图纸、零部件）的...
-->
...
- **核心能力**：6 类（版本管理 / 零部件库 / CAD 集成 / 检索与权限 / 变更管理 / 生命周期状态）
- **典型场景**：4 类（纯研发数据管理 / PDM 起步阶段 / 中小制造业 / CAD 文件 1 万-10 万）
- **上下游集成**：4 类（PDM-CAD/CAE/CAPP / PDM-PLM / PDM-ERP / PDM-MES）
- **PDM vs PLM**：范围 / 数据深度 / 流程 / 用户 / 集成 5 维度
- **主流厂商**：4 类（国际大厂 Siemens Teamcenter / Dassault ENOVIA / PTC Windchill / 国产 数码大方 / 华天）
- **选型维度**：6 类（CAD 集成 / 文件规模 / 行业 / 实施能力 / 升级路径 / TCO）
- **关键考量**：5 项（CAD 集成深度 / 零部件库设计 / 权限模型 / 变更流程 / 升级路径）
- **常见陷阱**：5 类（CAD 集成弱 / 权限设计过严 / 零部件命名乱 / 变更追溯断 / 升级失当）
