...
`ClassifierCompositeItemWriter` 只负责路由，每个委托 Writer 专注一个目标，便于单测和独立演进。注意它不会自动提供跨外部系统的原子事务：对于 HTTP、短信等非事务目标，应增加幂等键、Outbox 或补偿机制；相关可靠性设计参见[高可用设计](../../06.distributed-systems/03-high-availability/README.md)。
...
- [高可用设计](../../06.distributed-systems/03-high-availability/README.md) — 幂等、重试、降级与故障恢复
