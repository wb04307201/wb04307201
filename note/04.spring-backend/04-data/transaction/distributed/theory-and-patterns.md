...
module:
  ...
  slug: 04.spring-backend/04-data/transaction/distributed/theory-and-patterns
  type: article
  category: 主模块子文章
...
> 🎯 **一句话定位**：分布式事务 5 大方案 = 2PC/3PC/TCC/Saga/本地消息表，Spring 落地首选 Seata AT + RocketMQ 事务消息。

> 分布式事务理论（2PC/3PC/TCC/Saga/本地消息表）的深度讲解见 [12.interview/04.system-design/02-distributed/distributed-transaction/README.md](../../../../06.distributed-systems/02-distributed/distributed-transaction/README.md)，本节聚焦 Spring 集成。
...
> ⚠️ **版本说明**：Seata 1.4.x 是当前生产主流版本；2.x 引入了 Raft 集群模式与动态配置中心支持，新项目可直接选用 2.x。
...
- ⬅️ [04-multi-datasource](../../mybatis/03-spring-integration/04-multi-datasource.md) — MyBatis 多数据源配置（分布式事务常见触发场景）
- ⬅️ [failure-cases](../failure-cases.md) — 事务失效典型场景（含分布式场景）
- ⬅️ [multi-datasource-and-jta](../multi-datasource-and-jta.md) — 多数据源 JTA 集成详解
- ⬅️ [propagation-and-isolation](../propagation-and-isolation.md) — 事务传播与隔离级别（分布式下更需注意）
- ➡️ [circuit-breaker](../../../03-cloud/circuit-breaker.md) — 熔断降级与分布式事务联动
- ➡️ [seata-integration](../../../03-cloud/seata-integration.md) — Seata 微服务集成实战
