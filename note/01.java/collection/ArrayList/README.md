...
```java
// ❌ 陷阱 1: 想"独立修改 subList，不影响原列表"
List<Integer> list = new ArrayList<>(Arrays.asList(0, 1, 2, 3, 4));
List<Integer> sub = list.subList(1, 4);   // 这不是新列表，而是视图!
sub.set(0, 99);
// list 变为 [0, 99, 2, 3, 4] —— 原列表被悄悄修改

// ✅ 正确：显式拷贝，获得独立子列表
List<Integer> sub = new ArrayList<>(list.subList(1, 4));
sub.set(0, 99);
// list 仍为 [0, 1, 2, 3, 4]
```

```java
// ❌ 陷阱 2: 修改原列表后继续使用 subList
list.add(100);
sub.get(0);   // ConcurrentModificationException! (modCount 变化)
```

```java
// ❌ 陷阱 3: 试图把 subList 强转成 ArrayList
...
```java
// ❌ 反模式: Vector 的"线程安全"是假象 —— 单方法同步 ≠ 复合操作安全
Vector<String> vector = new Vector<>();
if (!vector.contains(element)) {   // 操作 1: 检查（获取锁后释放）
    vector.add(element);           // 操作 2: 添加（中间可能被插队）
}
```
...
// ✅ 正确做法 2: 换用并发容器，根本上消除锁
Set<String> set = ConcurrentHashMap.newKeySet();
set.addIfAbsent(element);          // 原子 check-and-set
```

**结论**：`Vector` 同步粒度在**方法级**，复合逻辑要靠外部同步 —— 推荐直接用 `CopyOnWriteArrayList` / `ConcurrentHashMap.newKeySet()` 等并发容器。
...
while (it.hasNext()) {
    if ("target".equals(it.next())) {
        it.remove();   // 安全：同步 expectedModCount
    }
}

// ✅ Java 8+ 推荐：removeIf 一行搞定
list.removeIf("target"::equals);

// ✅ Java 8+ Stream 收集（不改原列表）
...
```java
// ❌ 反模式 1: for-each 中直接调用 list.remove()
for (String s : list) {
    if ("target".equals(s)) {
...
// ❌ 反模式 2: 普通 for 循环下标删除 —— 可用但易错（元素左移会跳检）
for (int i = 0; i < list.size(); i++) {
    if ("target".equals(list.get(i))) {
        list.remove(i);   // 可能漏删，且仍触发 fail-fast（结构性修改）
        i--;              // 还得手动回退下标
    }
}
```
...
```java
// ❌ 反模式 1: 想"像 ArrayList 一样 add"
List<String> list = Arrays.asList("a", "b", "c");
list.add("d");   // UnsupportedOperationException!

// ❌ 反模式 2: 以为它是可变集合并 set 之后做结构性修改
list.set(0, "z");  // OK — set 走的是数组下标，结构不变
list.remove(0);   // UnsupportedOperationException!

// ✅ 正确做法 1: 拷贝为可变 ArrayList
List<String> mutable = new ArrayList<>(Arrays.asList("a", "b", "c"));
mutable.add("d");   // OK

// ✅ 正确做法 2: Java 9+ 用 List.of（真不可变，连 set 都不行）
List<String> immutable = List.of("a", "b", "c");
immutable.set(0, "z");  // UnsupportedOperationException!

// ✅ 正确做法 3: Java 10+ 用 List.copyOf（不可变副本）
List<String> copy = List.copyOf(Arrays.asList("a", "b", "c"));
```
...
- [LinkedList 双向链表与选型对比](../LinkedList/README.md) — 为什么 90% 场景该选 ArrayList
- [HashMap 深入](../../collection/hashmap.md) — Map 体系的入口（数组 + 链表 + 红黑树）
