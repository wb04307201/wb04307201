...
行为型模式关心**对象间职责分配与通信**——同一个请求，多个对象能同时响应；同一个状态变化，多个对象能同步更新。难点不在于选哪个模式，而在于**区分"通知谁"vs"通知什么"vs"谁来主导流程"**：观察者（Observer）解决"通知谁"，命令（Command）解决"通知什么"，状态（State）解决"谁主导流程"。本篇围绕这三条决策轴，对比 Observer/Mediator、Strategy/State、Command/Memento 等高频二选一。
