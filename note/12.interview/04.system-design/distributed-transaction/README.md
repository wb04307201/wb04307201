...
> **一句话定位**：跨库事务 4 套方案：2PC 强一致但慢、TCC 侵入大、Saga 长事务、本地消息表互联网主流。
...
用户下单：
1. 订单服务：创建订单 ✓
2. 库存服务：扣减库存 ✗（网络超时，失败了）

结果：订单创建了，但库存没扣。数据不一致！
```

微服务架构下，一个业务操作可能跨越多个服务。本地事务保证不了跨服务的一致性——这就是**分布式事务**问题。
...
- [分库分表分布式事务](../../03.database/README.md) — 分库分表场景下的跨库事务
