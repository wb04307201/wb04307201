...
> 系统设计高频面试题与难点深挖，对齐主模块 [`04.system-design`](../../../note/04.system-design/)。**21** 篇真题覆盖 MQ / 缓存一致性 / 缓存热点 Key / 限流 / 秒杀 / 分布式 ID / 事务 / CAP / 锁 / 幂等 / 熔断 / **微服务 vs 单体** / **搜索系统** / **搜索框 typeahead** / **多字段动态排名** / **文件上传** / **短链系统** / **分库分表** / **排查与运维** 19 大方向（find 校对 2026-08-20）。

---

## 文章清单（共 27 题，find 校对 2026-08-20）
...
| [搜索框 typeahead 系统设计](search-typeahead/) | ⭐⭐⭐⭐⭐ | Trie 树 + Redis ZSet + 实名商家权重 + 实时增量更新 + 5 大反模式 |
| [多字段动态排名](multi-field-ranking/) | ⭐⭐⭐⭐ | 归一化方法论 + 加权公式 + 动态权重 + Redis ZSet + 6 大场景 |
| [大文件上传系统](file-upload/) | ⭐⭐⭐⭐ | 分片 + 断点续传 + 秒传 + 对象存储 |
| [1→N 大文件分发系统设计](file-distribution-1toN/) | ⭐⭐⭐⭐⭐ | 带宽成本估算 + CDN/P2P/分片签名选型 + 断点续传 + 防盗链 + 5 大反模式 |
...
| 主题 | 难度 | 核心问题 |
|------|------|---------|
| [消息已读未读功能设计](message-read-status/) | ⭐⭐⭐⭐ | 数据库 + 缓存 + 推送 + 性能优化 |
| [媒体上传系统设计](media-upload/) | ⭐⭐⭐⭐ | 分片 + 断点续传 + 秒传 + 对象存储 |
| [砍一刀算法](砍一刀算法/) | ⭐⭐⭐⭐ | 助力活动系统设计 |

### 项目管理（已全部迁走，split-hairs 不再收此分类）

> 2026-06-30 路径整理：本类目的 3 题已**全部迁移**：
> - `app-quote-breakdown` / `outsourcing-pitfalls` → 新主模块 [`note/11.product-and-pm/`](../../11.product-and-pm/README.md)
...
> 📌 **2026-06-30 路径整理**：原项目管理的 3 题全部迁走（"5万 vs 50万 App 报价"、"外包避坑指南"→ [`note/11.product-and-pm/`](../../11.product-and-pm/README.md)；"App 技术栈选型"→ [`09.front-end/08-cross-platform/mobile-tech-stack/`](../../05.frontend/08-cross-platform/mobile-tech-stack/README.md)）。split-hairs 不再收"项目管理"类内容。
