...
场景：电商购物车页面，用户点击"清空"——
  for (let i = 0; i < 10000; i++) {
    localStorage.setItem(`item_${i}`, JSON.stringify(cart));
  }
结果：主线程阻塞 2 秒，页面完全无响应。
```

10000 次 localStorage 同步写入 vs 同等数据量 IndexedDB 异步写入（主线程无感知）—— 两种存储方式的体感差距达到 40 倍。Cookie 4KB 每次请求自动携带、localStorage 5MB 同步阻塞、IndexedDB 无上限但 API 复杂 —— 工程师面对四种存储方案经常凭直觉选错，直到用户投诉才发现是存储 API 在拖后腿。

为什么容量差 1000 倍，选型陷阱反而集中在"最方便"的那一个？
