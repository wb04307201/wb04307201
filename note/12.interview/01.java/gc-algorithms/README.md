...
> **一句话定位**：从标记-清除到 ZGC 的演进脉络 —— Serial/Parallel 拼吞吐、CMS/G1 拼停顿、ZGC 拼亚毫秒延迟。
...
// 你的应用运行了几天后，越来越慢...
// GC 日志：
// [Full GC (Ergonomics) 2.1G->1.9G:4.5G, 3.2 secs]  ← 停顿 3 秒！
// [Full GC (Allocation Failure) 3.5G->3.3G:4.5G, 5.1 secs]  ← 停顿 5 秒！
```

每次 Full GC 都停顿好几秒——对用户来说就是"卡住了"。

为什么 GC 这么慢？为什么有的收集器快有的慢？Serial、Parallel、CMS、G1、ZGC 到底该怎么选？

理解 **GC 算法的演进脉络**，才能做出正确的选择。
