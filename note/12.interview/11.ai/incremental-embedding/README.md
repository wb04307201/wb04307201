...
> 一句话定位：**全量重编 = O(N) 成本 + 一致性问题；增量更新 = 消息队列 + 异步编码 + 索引同步 + 版本管理**。完整深度 + 模型版本切换方案见 [主模块 · vector-search-at-scale 第 5.1 节深度扩展](../vector-search-at-scale/README.md)。
...
- [11.ai · vector-search-at-scale 第 5.1 节深度](../vector-search-at-scale/README.md) — 18 行速查 + 本篇新增深度章节
