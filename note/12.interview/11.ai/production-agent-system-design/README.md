...
> **一句话定位**：生产 Agent 端到端 = 8 步流水线串 8 大组件（需求 → 架构 → Harness → Loop → 可靠性 → Memory → 评测 → 监控）。

← 返回 [11.ai 面试题](../README.md)

---

## 引子：面试官要你 90 秒画出 Agent 端到端流水线

```text
面试官：设计一个生产级 Agent 系统，你怎么从头到尾搭？
你：用 ReAct + Function Calling + Memory……
面试官：组件我都知道。问题是——怎么把 8 大组件串成流水线？
        Harness 怎么设计？可靠性防线怎么加？评测怎么做？
你：……
```

知道 8 大组件只是起点，面试官真正考察的是端到端工程能力：8 步流水线（需求分析→架构选型→Harness→Loop+Verifier→可靠性→Memory→评测→监控），Harness 的 4 大约束类型（Prompt / 工具 / 输出 / 流程），可靠性 4 层防线（L1 Prompt 约束 → L2 Verifier → L3 状态回滚 → L4 成本熔断），6 维评测体系的权重分配 —— 怎么在 90 秒内把这些串起来讲清楚？
