...
> ⬅️ [返回 AI 咬文嚼字](../README.md) | [主模块深度专题](../../../09.ai-applications/rag/knowledge-ingestion-pipeline/README.md)

> **一句话定位**：**4 大核心知识入库面试深挖**：多源连接 / 复杂文档解析 / 去重与 CDC / 亿级 Chunk 性能。

---

## 引子：12 小时 vs 25 分钟的入库差距

```text
场景：企业 RAG 知识库首次全量入库——
  数据量：100 万篇文档 → 约 1000 万 Chunk
  方案 A：同步逐条 insert + 串行 Embedding → 12 小时
  方案 B：bulk 10000 条/批 + async 队列 + GPU 批量 Embedding → 25 分钟
```

差距 29 倍，但绝大多数团队的第一版都是方案 A。知识入库的坑远不止性能 —— Confluence / GitHub / 数据库等 10+ 数据源的连接器选型、PDF 表格与 OCR 的 5 大解析雷区、URL + 内容 hash + 向量近似的 3 层去重，任何一个踩中都可能让整条流水线返工。

4 道高频面试题，把"能跑通"和"能上线"之间的距离一次性讲透。
...
- **主模块深度**：[企业级 Knowledge Ingestion Pipeline](../../../09.ai-applications/rag/knowledge-ingestion-pipeline/README.md) — 8 阶段全链路：连接、解析、清洗、分块、Embedding、入库与 CDC。
...
> 📅 2026-09-01 · 咬文嚼字 · 知识入库 · ⭐⭐⭐⭐⭐（高频面试 + 实战必会）
