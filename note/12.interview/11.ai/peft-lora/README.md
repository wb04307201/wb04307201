...
## 引子：训练 7B 模型，显存从 65GB 降到 2GB

```text
全参微调 LLaMA-7B：65GB 显存（需 A100-80G）
LoRA r=8 微调：     2GB 显存（3090 即可）
效果差距：          ≤ 1%（AlpacaEval）
```

参数量从 67 亿降到 13 万（压缩 500 倍），效果几乎不变 —— 但面试官不会只问"低秩分解"。B 矩阵为什么必须全零初始化？α/r 比值取 1 还是 2？QLoRA 的 NF4 凭什么比 INT4 误差小 50%+？DoRA 把权重拆成 magnitude × direction 后为什么在 HumanEval 上比 LoRA 强 3.2 个百分点？PEFT 背后的数学与工程细节，远比"省显存"三个字复杂。
...
> 📅 2026-09-01 · 咬文嚼字 · PEFT/LoRA · ⭐⭐⭐⭐⭐（高频面试 + 实战必会）
