...
```python
# 直觉：KV Cache 只是"缓存中间结果"，占不了多少显存
# 实测（70B 模型 × 128k 上下文）：
#   模型权重（fp16）：~140GB
#   KV Cache：        320GB+   ← 比权重还大
# 而且 LLaMA-2 从 MHA 换成 GQA 后：
#   KV 显存直降 4x，精度损失 < 0.5%
```

同样是"共享 KV"，MQA 让所有头共享 1 份、GQA 让 n 个头分组共享，MLA 干脆用低秩压缩重写规则——三者的训练成本与精度代价完全不同。更反直觉的是：KV Cache 不是常数级优化，它把自回归生成的复杂度从 O(n³) 拉回 O(n²)。这三个字母到底各解决什么问题？
...
> 📅 2026-09-01 · 咬文嚼字 · KV Cache · ⭐⭐⭐⭐（高频面试 + 实战必会）
