...
> ⬅️ [返回 AI 咬文嚼字](../README.md) | 主模块深度专题

> **一句话定位**：5 大 LLM 对齐方法**面试深挖版**：SFT / RLHF / DPO / Constitutional AI / SimPO。

---

## 引子：同样训一个对齐模型，成本差 10 倍

```text
场景：团队要让 7B 开源模型对齐人类偏好——
  方案 A (RLHF)：actor + critic + ref + RM 4 个模型，
                 LLaMA-2 参考：1024 张 A100 × 21 天，HumanEval 29%→48%
  方案 B (DPO) ：policy + ref 2 个模型，
                 2 卡 24G 即可跑，AlpacaEval 2.0 差距 < 2%
  方案 C (Constitutional AI)：50 条宪法 + AI 自评估，标注成本 $1M→$100K
```

4 模型协同 vs 2 模型闭式解 vs AI 自我批评 —— 三条路线的工程复杂度和训练成本差距巨大，但在 7B 模型上的最终效果差异不到 2%。面试时答"先用 SFT 再 RLHF"只是起点，真正的分水岭在于：**什么规模用什么方法，什么预算选什么组合**。

5 道核心题，把对齐方法的"能用"和"该用"一次性拆开。
...
- **主模块深度**：llm-alignment — 5 大方法完整版
- **兄弟面试题**：[Agent 性能评估](../agent-performance-evaluation/README.md) 顺带提
- **餐厅叙事**：[12.story 系列目录](../../../13.story/README.md)（对齐主题餐厅叙事篇待补）
...
> 📅 2026-09-01 · 咬文嚼字 · LLM 对齐 · ⭐⭐⭐⭐⭐（高频面试 + 实战必会）
