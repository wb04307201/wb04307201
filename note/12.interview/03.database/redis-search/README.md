...
```text
周五 18:30，告警群炸锅：
- Redis 集群 P99 延迟从 0.5ms 飙升到 47000ms
- 所有业务线超时，订单服务、用户服务全部不可用
- 根因：实习生在 1 亿键的生产环境执行了 KEYS "user:*"
- KEYS 遍历全部 1 亿键，阻塞 Redis 单线程 47 秒
```

一条看起来"无害"的模式匹配命令，在数据量膨胀后变成了生产事故。而 `SCAN` 同样能匹配键名，却因游标分批迭代把单次阻塞控制在微秒级——结果一样，代价天壤之别。问题是：**KEYS 和 SCAN 的本质区别是什么？1 亿键规模下还有哪些搜索方案？什么时候该引入 RediSearch？**
