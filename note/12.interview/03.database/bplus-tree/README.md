...
> **一句话定位**：B+ Tree 凭"非叶子只存键 + 叶子链表"两招，3 层即可管 1 亿行，是数据库索引的事实标准。
...
> "先说**为什么不用别的结构**：二叉/红黑树高度太高（上亿数据要几十层，几十次随机 IO）；哈希索引不支持范围和排序；B-Tree 数据分散在所有节点、范围查询要回溯。B+ Tree 正好补齐这些。
>
> **扇出与树高**：非叶子节点只存'键 + 子节点指针'，不存数据行，所以一个 16KB 页能放上千个键，扇出 1000+。三层树 ≈ 1000³ = 10 亿行，且根节点常驻内存，实际查询 2~3 次磁盘 IO。
>
> **范围查询**：所有数据都在叶子层，叶子之间用双向链表连接，`WHERE age BETWEEN 20 AND 30` 定位起点后顺序扫链表即可，天然有序、无需回溯。
>
> **聚簇 vs 二级索引**：InnoDB 主键索引（聚簇）叶子直接存整行；二级索引叶子存主键值，命中后要拿主键**回表**再查一次聚簇索引。若 SELECT 的列都被索引覆盖（覆盖索引），就免了回表。
>
> **工程坑**：自增主键顺序写入、页利用率高；UUID/雪花 ID 等随机主键导致频繁**页分裂**和碎片，写放大严重——这也是'为什么建议用自增主键'的根因。"
