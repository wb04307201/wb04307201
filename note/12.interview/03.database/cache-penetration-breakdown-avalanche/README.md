...
> **一句话定位**：穿透/击穿/雪崩是缓存三连击，根因都是缓存失守让 MySQL 独扛所有流量。

## 引子：数据库突然被打挂了

```text
凌晨 2 点，告警：MySQL CPU 100%！
```

你检查了一下：Redis 好好的，没有宕机。但所有请求都穿透到了 MySQL。

三种可能：
1. **缓存穿透**：有人疯狂查询一个**不存在的 key**（比如 ID = -1）
2. **缓存击穿**：一个**热点 key 突然过期**，所有请求同时打到 MySQL
3. **缓存雪崩**：**大量 key 同时过期**，或者 Redis 集群宕机
