...
> **一句话定位**：Sentinel 解决高可用（主从+故障转移），Cluster 通过 16384 哈希槽解决水平扩展，两者不能混为一谈。
...
1. **主从复制**：一主多从，读写分离（但从机数据可能滞后）
2. **Sentinel 哨兵**：主从 + 自动故障转移（解决高可用，但不能水平扩展）
3. **Cluster 分片集群**：数据分片 + 去中心化（既能扩展，又能高可用）
