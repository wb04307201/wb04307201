...
```text
同一台 MySQL，同一条 SQL：
- 上线初期：EXPLAIN → type=ref, rows=500, 耗时 200ms
- 三个月后：EXPLAIN → type=ALL, rows=100000, 耗时 5s
- 数据量从 5 万涨到 100 万，执行计划突变
```

数据量增长本是业务好事，但优化器悄悄从"走索引"切换到"全表扫描"——不是索引被删了，而是**统计信息过期 + 数据分布偏移**让优化器误判了成本。更隐蔽的是 `YEAR(create_time)=2024` 这类写法，看起来语义清晰，却因对列施加函数直接废掉索引，而 `LIMIT 1000000, 10` 即使有主键索引也要先扫 100 万行再丢弃前 999990 行。SQL 调优的难点从来不是"加个索引"，而是**定位 → 分析 → 优化 → 验证**的系统方法论。
