...
> 大数据高频面试题与深度剖析：实时计算 / 批处理 / 数据仓库 / 数据湖 / OLAP / 消息队列 6 组，覆盖 80% 大数据面试高频题。

---

## 文章清单（共 6 篇，find 校对 2026-08-25）

> 主模块深度均见 [`03.data-stack/02-big-data`](../../03.data-stack/02-big-data/)（Kafka Exactly-Once 另见 [`06.distributed-systems`](../../06.distributed-systems/)），下表不再重复标注。
...
1. **入门**（3-5 天）：Hive 分区 vs 分桶 + Doris vs ClickHouse 选型 + Flink Checkpoint vs Savepoint
2. **进阶**（1-2 周）：Spark Shuffle 优化 + Kafka Exactly-Once + Iceberg ACID
3. **冲刺面试**（2-3 天）：全部 6 篇的面试话术 + 交叉引用串联
