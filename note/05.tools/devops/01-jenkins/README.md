...
| 维度 | Jenkins | GitHub Actions | GitLab CI | CircleCI |
|------|---------|----------------|-----------|----------|
| **部署方式** | 自托管（物理机/K8s/Docker） | 云托管（GitHub 内置） | 自托管 / GitLab SaaS 托管 | 云托管（SaaS） |
| **配置语言** | Groovy（Jenkinsfile） | YAML（.github/workflows/） | YAML（.gitlab-ci.yml） | YAML（.circleci/config.yml） |
| **插件生态** | ⭐⭐⭐⭐⭐ 1800+ 插件 | ⭐⭐⭐⭐ GitHub Marketplace（Actions） | ⭐⭐⭐ 中等 | ⭐⭐⭐ 中等 |
| **分布式构建** | ✅ Master-Agent（可扩展到千节点） | ✅ 自托管 Runner | ✅ GitLab Runner | ✅ 有限（Orb 机制） |
| **价格** | 免费（自托管硬件/运维成本） | 免费 2000 min/月 + $0.008/min 超出 | 免费 400 min/月 + $10/月 10000 min | 免费 30000 min/月 + $15/月 超出 |
| **学习曲线** | 陡峭（Groovy + 插件体系） | 低（YAML + 文档好 + Marketplace） | 中等（YAML + CI/CD 一体化） | 低（YAML + Orb 复用） |
| **K8s 集成** | ✅ 深度（K8s Plugin + 动态 Agent Pod） | ⭐⭐⭐ 一般（需自托管 Runner） | ⭐⭐⭐ 一般（需 GitLab Runner） | ⭐⭐⭐ 一般 |
| **适用团队规模** | 大型（100+ 人 / 复杂定制） | 中小型（GitHub 用户 / 快速上手） | 中小型（GitLab 用户 / 一体化） | 中小型（快速交付 / 简单流水线） |

**选型建议**：
- 大型企业 + 复杂定制 → **Jenkins**（插件最丰富、Master-Agent 分布式）
- GitHub 用户 + 快速上手 → **GitHub Actions**（零配置、Marketplace 丰富）
- GitLab 用户 + 一体化 → **GitLab CI**（代码 + CI 同平台、MR 触发）
- 中小型团队 + 简单流水线 → **CircleCI**（配置简洁、Orb 复用）
...
> **前置条件**：
> - Java 17+（Jenkins 2.361+ 要求）
> - 最低 256MB 内存（推荐 1GB+）
> - 磁盘 >= 10GB（构建产物 + 插件缓存）
> - 网络能访问 Jenkins 镜像源（mirrors.jenkins.io）
