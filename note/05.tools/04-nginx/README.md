...
| **性能** | ⭐⭐⭐⭐⭐ 极高（10万+ QPS） | ⭐⭐⭐ 中等 | ⭐⭐⭐⭐⭐ 极高（专攻 L4/L7 负载均衡） | ⭐⭐⭐⭐⭐ 极高（云原生优化） | ⭐⭐⭐⭐ 高 |
| **内存占用** | 低（~10MB/worker） | 高（~50MB/进程） | 极低（~5MB） | 中（~50MB） | 低（~20MB） |
| **配置语言** | 自有 DSL（nginx.conf） | 自有 DSL（httpd.conf） | 自有 DSL（haproxy.cfg） | YAML + xDS 动态下发 | Caddyfile（极简） |
| **动态配置** | 需 reload（有限支持 API） | 需 reload | 需 reload（Stats socket 有限） | ✅ 热更新（xDS API） | ✅ 热更新（API + 自动 HTTPS） |
| **HTTP/3** | ✅ 1.25+ 原生支持 | ❌ 不支持 | ❌ 不支持 | ✅ 支持 | ✅ 原生支持 |
| **生态/插件** | 丰富（开源 + Nginx Plus 商业版） | 极丰富（模块最多） | 专注负载均衡，模块少 | 云原生生态（Istio/K8s 深度集成） | 中等（Go 插件） |
| **学习曲线** | 中等 | 中等 | 中等 | 陡峭（需理解 xDS/控制面） | 低（配置最简） |
| **适用场景** | 通用反向代理 / 静态服务 / API 网关 | 传统企业 / 复杂模块（.htaccess） | 纯 L4/L7 负载均衡 / TCP 代理 | 云原生 / 服务网格 / K8s Ingress | 快速部署 / 自动 HTTPS / 小团队 |

**选型建议**：
- 通用反向代理 + 静态服务 → **Nginx**（最成熟、性能极致）
- 云原生 / K8s 服务网格 → **Envoy**（xDS 动态配置、Istio 默认）
- 纯四层/七层负载均衡 → **HAProxy**（性能极致、专注）
- 快速上手 + 自动 HTTPS → **Caddy**（配置最简、零证书管理）
- 传统企业 .htaccess 需求 → **Apache**（模块最多、兼容性好）
