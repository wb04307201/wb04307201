...
| `02-docker/`（本文） | 1 | 顶层 |
| ├─ `command/` | 1 | 命令速查 |
| ├─ `docker-compose/` | 1 | Docker Compose 编排实战 |
| ├─ `images/` | 1 | 常用镜像构建 |
| └─ `podman/` | 1 | Podman 替代方案 |
| **分类 leaf 合计** | **4 depth-2 + 1 顶层 = 5** | 100% frontmatter |
| **学习路径主题数** | 2 条（入门：速查 → 进阶：Compose + Podman） | 见上方学习路径 |
