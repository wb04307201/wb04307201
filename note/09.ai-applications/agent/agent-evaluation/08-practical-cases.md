...
| 维度 | DAG | ReAct | 选型建议 |
|------|-----|-------|---------|
| **平均步数** | 3-5 步（确定性路径） | 5-10 步（含思考/观察） | 简单任务用 DAG |
| **最优步数比** | 1.0-1.5 | 1.5-3.0 | 追求效率用 DAG |
| **灵活性** | 低（预定义路径） | 高（动态决策） | 未知场景用 ReAct |
| **调试难度** | 低（可追踪 DAG 图） | 高（reasoning 链长） | 生产环境用 DAG |
| **容错能力** | 需预设分支 | 可自主重试 | 高失败率用 ReAct |

**核心公式**：

```text
步骤效率比 = 最优步骤数 / 实际步骤数（越接近 1.0 越好）
冗余步骤率 = (实际步骤 - 最优步骤) / 实际步骤 × 100%
```
...
```text
单任务成本 = 总 Token × 单价 / 任务数
成本效益比（ROI）= 任务价值 / 单任务成本（> 5 才划算，> 10 优秀）
Token 效率 = 总 Token / 任务数（越低越好，关注冗余上下文）
```
...
## 选型建议：按团队规模选方案
| 场景 | 推荐方案 | CI/CD 工具 | 黄金集规模 | 阈值 |
|------|---------|-----------|-----------|------|
| **MVP（1-3 人）** | GitHub Actions + 自动化指标 | 简单 YAML | 50 条 | ≥ 0.80 |
| **增长（5-10 人）** | 多模型 Judge + A/B Test | GitHub/GitLab CI | 200 条 | ≥ 0.85 |
| **成熟（> 10 人）** | 漂移检测 + 自动回滚 | GitLab CI 多阶段 | 500+ 条 | ≥ 0.85 |
| **高合规（金融）** | 人工审核 + 审计日志 | 自建 Pipeline | 100% 覆盖 | ≥ 0.99 |

**一句话原则**：先跑通自动化指标（成本最低），再逐步加 LLM-Judge → A/B → 漂移检测。
