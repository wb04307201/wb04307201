...
- **循环调用**：Loop Engineering（⚠️ 待 Phase 1+ 迁入；占位 `[../../agent-execution-patterns/loop-engineering/]`） — Ralph Wiggum Loop（Codex 也支持 `/goal` 长任务，pre-existing in `note/`，保持 unstaged）
