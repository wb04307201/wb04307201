...
Camunda 8 的关键不是语法——是**看起来对**的代码背后那些'踩坑点'。
...
```mermaid
%% [AUTO] 占位 — 描述：Camunda Platform 8 开源架构（Self-Managed）— Zeebe + Operate + Tasklist + Optimize 4 大组件
%% 实际图：img_5.png（保留 PNG 用于参考）
graph TB
...
```mermaid
%% [AUTO] 占位 — 描述：免费使用 Camunda 8 源码构建生产环境路径 — 全开源无商业许可费的部署拓扑
%% 实际图：img_4.png（保留 PNG 用于参考）
graph TB
    %% TODO: 人工设计等价 Mermaid 替换
    A[Component A] --> B[Component B]
    B --> C[Component C]
