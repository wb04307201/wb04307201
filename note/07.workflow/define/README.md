...
工作流定义 的关键不是语法——是**看起来对**的代码背后那些'踩坑点'。
...
| 概念 | 一句话定义 | 何时用 |
|:-----|:----------|:------|
| **工作流（Workflow）** | 业务目标的有序、可重复活动集合 | 所有业务协作场景 |
| **BPMN 2.0** | OMG 业务流程建模国际标准（XML + 图形化）| 跨组织/跨厂商流程建模 |
| **流程定义（Process Definition）** | 工作流的**静态蓝图**（BPMN XML）| 一次定义，N 次实例化 |
| **流程实例（Process Instance）** | 工作流的**动态执行**（一次具体审批）| 跟踪具体业务对象 |
| **任务（Task）** | 流程实例中**最小执行单元** | 流程图节点 = 任务 |
| **流程变量（Process Variable）** | 任务间传递的业务数据 | 上下文/数据流 |
| **PVM** | 流程虚拟机（jBPM 引入）| Camunda 7 架构基础 |
| **编排（Orchestration）** | 微服务/系统层面的工作流 | 跨服务协调 |

**关键区分**：

- **流程定义 vs 流程实例**：定义是模板（Java class），实例是对象（Java instance）
- **任务 vs 节点**：任务一定是 Activity 类型的节点；节点还包括 Event / Gateway
- **OA 审批 vs 工作流**：OA 是工作流的子集（仅人事审批），工作流覆盖所有协作

> 📌 BPMN 22 元素完整速查见 [章节根 README §BPMN 元素速查](../README.md#bpmn-元素速查22-元素)。
