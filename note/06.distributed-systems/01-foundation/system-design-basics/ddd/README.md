...
> **定位**：领域驱动设计（Domain-Driven Design，简称DDD）是一种以业务领域为核心，通过深入理解和建模业务逻辑来指导软件设计与开发的系统性方法论。 的核心原理、实现与最佳实践。
...
DDD 分为两大设计层次，二者紧密配合：**战略设计回答”系统边界在哪”，战术设计回答”边界内如何建模”**。

| 维度 | 战略设计（Strategic Design） | 战术设计（Tactical Design） |
|------|--------------------------|--------------------------|
| **关注点** | 系统边界、业务全景 | 边界内的领域模型与代码 |
| **核心概念** | 限界上下文、子域、上下文映射 | 实体、值对象、聚合根、领域服务、领域事件 |
| **产出** | 限界上下文图、子域划分 | 领域模型、聚合设计、代码骨架 |
| **参与者** | 业务专家 + 架构师 + 团队负责人 | 业务专家 + 开发人员 |
| **时间维度** | 项目早期，全局视角 | 建模阶段，迭代细节 |
| **失败代价** | 拆错服务边界（代价巨大） | 单服务内部模型不优雅（代价可控） |

### 战略设计

- **限界上下文（Bounded Context）**：模型一致性的边界，每个上下文内部用统一语言。
- **子域（Subdomain）**：核心子域（业务核心竞争力）、支撑子域（业务必需但非核心）、通用子域（可复用）。
- **上下文映射（Context Mapping）**：合作关系、共享内核、客户-供应商、防腐层（ACL）、开放主机服务（OHS）等。

> 战略设计的产物是**服务边界**——一个限界上下文通常对应一个微服务。详见 [微服务与 DDD](../microservices/microservices-and-ddd/README.md) 与 [微服务拆分策略](../microservices/service-decomposition/README.md)。

### 战术设计

- **实体（Entity）**：有唯一标识、连续性的对象。
- **值对象（Value Object）**：无标识、不可变、由属性定义。
- **聚合（Aggregate）+ 聚合根**：一致性边界，外部只能通过聚合根操作。
- **领域服务（Domain Service）**：跨实体/聚合的业务逻辑。
- **领域事件（Domain Event）**：已发生的业务事实，用于解耦。
...
> **为什么要先战略后战术？**——如果边界错了（战略错），再精致的战术模型也是浪费——边界内可能需要推倒重来。先用战略设计确定”对的边界”，再在每个边界内做战术建模，是**风险最小、收益最大**的实践路径。
