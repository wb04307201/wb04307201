...
| 机制 | 说明 | 配置示例 |
|------|------|---------|
| **同步发送 + 重试** | 发送失败自动重试 3 次，确保消息到达 Broker | RocketMQ: `retryTimesWhenSendFailed=3` |
| **生产者 ACK** | Broker 确认消息已持久化后才返回成功 | Kafka: `acks=all` |
| **事务消息** | 本地事务 + 消息发送原子性，避免"事务成功但消息没发" | RocketMQ: `sendMessageInTransaction()` |

#### Broker 端可靠性

| 机制 | 说明 | 配置示例 |
|------|------|---------|
| **同步刷盘** | 消息写入磁盘后才返回 ACK，防止宕机丢消息 | RocketMQ: `flushDiskType=SYNC_FLUSH` |
| **多副本同步** | 主从同步复制，主节点挂了从节点有完整数据 | Kafka: `min.insync.replicas=2` |
| **消息持久化** | 所有消息写磁盘，不依赖内存 | RabbitMQ: 队列 + 消息都设为 durable |

#### 消费端可靠性

| 机制 | 说明 | 注意事项 |
|------|------|---------|
| **手动 ACK** | 消费成功后才确认，失败则重新投递 | 不要在 catch 块中 ACK，否则异常消息被丢弃 |
| **重试 + 死信队列** | 重试 N 次后进死信队列，人工/自动补偿 | 死信队列必须接告警，否则丢消息无人知晓 |
| **幂等消费** | 同一消息处理多次，结果一致 | 先查幂等表 → 不存在才处理 → 成功后写幂等（事务内） |

#### 全链路排查框架

当出现"消息丢失"时，按以下 5 步定位：

```text
Step 1: 生产端是否发出？ → 查发送日志 + 回调接收日志
Step 2: MQ 是否收到？    → 查消息轨迹 / offset / lag
Step 3: 消费者是否拉取？ → 查消费日志 + 消费者健康状态
Step 4: 消费逻辑是否成功？→ 查业务日志 + DB 状态
Step 5: 是否被幂等误拦截？→ 查幂等表 + 对比订单表
```

> **生产建议**：不能只依赖被动回调，要做**主动轮询兜底** — 定时查支付网关确认状态。全链路 traceId 贯穿（回调 → MQ → 消费 → DB），是排查的基础设施。
...
<!-- TODO: 拆分候选 (518 行 / 11 个 H2，超 500+8 阈值） -->
