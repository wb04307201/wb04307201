...
> **一句话定位**：技术债 = 短期捷径的长期利息——本文用 Fowler 四象限 + 价值-成本矩阵给出识别、量化与偿还的完整管理流程。
...
```mermaid
quadrantChart
    title Martin Fowler 技术债四象限
    x-axis "谨慎" --> "鲁莽"
    y-axis "无意" --> "有意"
    quadrant-1 "鲁莽+有意：'没时间设计'"
    quadrant-2 "谨慎+有意：'先上线再还债'"
    quadrant-3 "鲁莽+无意：'什么是分层？'"
    quadrant-4 "谨慎+无意：'现在知道怎么做了'"
```
...
graph TD
    TD["技术债"]
    TD --> Code["代码债<br/>命名不规范/重复代码/过长方法"]
    TD --> Test["测试债<br/>缺少单元测试/覆盖率低"]
    TD --> Design["设计债<br/>架构不合理/耦合过紧"]
    TD --> Infra["基础设施债<br/>手动部署/缺少CI/CD"]
    TD --> Doc["文档债<br/>文档缺失/过时"]
```
...
| 时期 | 里程碑 | 关键实践 |
|------|--------|----------|
| 1992 | Ward Cunningham 首次提出"技术债"隐喻 | 用金融类比解释代码质量权衡 |
| 2008 | Martin Fowler 发布技术债四象限 | 按鲁莽/谨慎 × 有意/无意分类 |
| 2019 | Strangler Fig 模式普及 | 渐进式偿还架构债，避免大爆炸重写 |
| 2020s | SonarQube / CodeClimate 量化技术债 | 以分钟数/金额量化债务，纳入 CI 门禁 |
| 2024+ | AI 辅助债务检测 | LLM 自动识别代码异味并建议重构方案 |

## 技术债量化指标

- **行业基准**：每个迭代分配 **20%** 工作量用于偿还技术债（Martin Fowler 推荐）
- **SonarQube 阈值**：单个模块技术债 ≤ 30 min 为健康；超过 2 h 需列入还债计划
- **代码质量门禁**：新增代码技术债 ≤ 5 min/PR，超出则 CI 阻断合并

## 管理流程典型产出物

| 阶段 | 典型产出物 |
|------|-----------|
| 识别 | 技术债登记册（Backlog 条目 + 发现人 + 日期） |
| 可视化 | 价值-成本矩阵图 + 技术看板卡片 |
| 分析优先级 | 优先级评分表（技术价值 × 业务价值 / 成本） |
| 计划执行 | 迭代 Backlog 中 20% 技术改进任务 |
| 总结回顾 | 技术债偿还报告（已还/剩余 + 趋势图） |

## 管理技术债
Martin Fowler 在 Is High Quality Software Worth the Cost? 一文中从添加功能 / 时间两个维度来对比高质量的软件和低质量的软件在交付速率上的差别。  
```mermaid
graph LR
    subgraph 高质量软件["高质量软件"]
        A1["功能交付速率 → 稳定"]
    end
    subgraph 低质量软件["低质量软件（高技术债）"]
        B1["功能交付速率 → 持续下降"]
    end
    Time["时间 →"] --> A1
    Time --> B1
...
graph TD
    subgraph 价值映射["价值映射图"]
        TV["技术价值"] --> Score["改进优先级评分"]
        BV["业务价值"] --> Score
    end
    Score --> Card["创建技术卡<br/>可视化改进任务"]
    Card --> Action["按优先级执行"]
```
...
- [架构治理 ADR](../system-design-basics/togaf/architecture-governance.md) — 用架构决策记录追踪技术债的演进与偿还
- [康威定律与组织债](../system-design-basics/togaf/conway-and-team-topology.md) — 组织结构对技术债积累的影响
