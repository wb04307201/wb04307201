...
> **一句话定位**：CDN = **全球边缘节点缓存静态资源 + DNS 智能调度就近访问 + 回源策略处理 miss**，显著降低延迟、减轻源站压力。

Content Delivery Network 内容分发网络——通过将静态资源缓存到全球边缘节点，让用户就近获取，显著降低延迟与源站压力。
...
5. 最简单的办法:Referer 防盗链 和 时间戳防盗链

## CDN 协议演进：HTTP/2 → HTTP/3

| 对比维度 | HTTP/2 | HTTP/3 (QUIC) |
|---------|--------|--------------|
| 传输层 | TCP | QUIC (UDP) |
| 队头阻塞 | TCP 层仍有队头阻塞 | 完全消除队头阻塞 |
| 建连延迟 | TCP + TLS = 2 RTT | 0-RTT 或 1-RTT 建连 |
| 弱网表现 | 丢包后整条连接阻塞 | 独立流恢复，弱网更稳定 |
| CDN 收益 | 多路复用 + 头部压缩 | 首屏加速 + 弱网优化，适合移动端 |
| 主流 CDN 支持 | 全面支持 | Cloudflare / 阿里云 / AWS CloudFront 已支持 |
...
- **媒体专项**：[media-upload-storage](../media-upload-storage/README.md) — CDN 在图片视频场景的边缘优化（HTTP/3 + WebP/AVIF + HLS LL-Latency）
