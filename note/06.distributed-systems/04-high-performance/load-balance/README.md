...
> **一句话定位**：负载均衡 = 将请求分发到多台服务器，提升并发能力与可用性。硬件方案（F5/A10）性能强但价格高，软件方案（Nginx/LVS/HAproxy）性价比高，客户端方案（Spring Cloud LoadBalancer）用于微服务内部调用。
## 目录

- [一、负载均衡分为哪几种](#一负载均衡分为哪几种)
- [二、负载均衡常见的算法](#二负载均衡常见的算法)
- [三、相关章节](#三相关章节)
...
| 场景 | ❌ 错误做法 | ✅ 正确做法 |
|------|-----------|-----------|
| 长连接场景 | 所有场景用轮询 → 长请求堆积在慢服务器 | 用最少连接（Least Connections） |
| 性能不均 | 所有服务器等权分配 | 加权轮询，按 CPU/内存比例设 weight |
| 有状态服务 | 轮询导致 Session 丢失 | ip_hash 或 Sticky Cookie |
| 热点数据 | URL 哈希导致热点服务器过载 | 一致性哈希 + 虚拟节点打散热点 |
| 突发流量 | 固定算法不感知后端压力 | 最少响应时间 + 健康检查联动 |

### LVS vs Nginx vs HAProxy 演进对比

| 对比维度 | LVS | Nginx | HAProxy |
|---------|-----|-------|---------|
| 工作层 | L4（内核级） | L7（应用层） | L4 + L7 |
| 性能 | 最高（内核转发） | 中（用户态进程） | 高（事件驱动） |
| 典型版本 | LVS 2.6+（Linux 内核内置） | Nginx 1.9+（stream 模块支持 L4） | HAProxy 2.0+（多线程） |
| 适用场景 | 入口流量分发、DB 集群 | Web 反向代理、API 网关 | 数据库代理、高可用集群 |
| 配置复杂度 | 高（需 ipvsadm） | 低（nginx.conf） | 中（haproxy.cfg） |

## 三、相关章节

- [缓存设计模式](../cache-patterns/README.md) — 缓存层的负载均衡策略
- [API 网关](../../02-distributed/api-gateway/README.md) — API 网关统一接入层的负载均衡
