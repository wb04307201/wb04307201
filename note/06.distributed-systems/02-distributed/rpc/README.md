...
> **一句话定位**：RPC 用 Stub 与 Skeleton 屏蔽网络细节，让远程调用像本地函数一样自然；gRPC、Dubbo、Thrift 是主流框架。
...
sequenceDiagram
    ...
    C->>Stub: 调用本地方法
    Stub->>Stub: 序列化（方法名 + 参数）
    Stub->>Net: 发送请求（TCP/HTTP/2）
    Net->>Skel: 传输到服务端
    Skel->>Skel: 反序列化参数
    Skel->>S: 调用实际方法
    S-->>Skel: 返回结果
...
## 关键组件- **序列化/反序列化**：将数据转换为可传输格式（如JSON、Protobuf、Thrift）。
...
## RPC vs REST/HTTP API| **特性**   | **RPC**             | **REST/HTTP API** |
|----------|---------------------|-------------------|
...
## 典型应用场景- **微服务架构**：服务间调用（如订单服务调用库存服务）。
...
## 发展趋势- **gRPC主导**：HTTP/2 + Protobuf成为高性能RPC标准。
- **Service Mesh集成**：通过Istio/Linkerd管理RPC流量（如熔断、重试）。
- **Serverless RPC**：AWS Lambda、阿里云函数计算等无服务器架构支持RPC调用。

## 总结RPC是分布式系统的核心通信方式，适合内部服务高效协作。选择框架时需权衡性能、语言支持、生态成熟度等因素。对于公开API，REST/GraphQL可能更合适；而内部微服务间通信，gRPC或Dubbo是更优选择。
