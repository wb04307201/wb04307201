...
module:
  ...
  slug: system-design/redundancy-design
  type: article
  category: 主模块子文章
  summary: 冗余设计 本应该很简单
-->
...
> **一句话定位**：冗余设计是 HA 的物质基础——数据/服务/地理多副本与故障转移的工程实践。

在分布式系统中，冗余设计是提升系统可靠性和可用性的核心容错策略，其核心思想是通过增加数据或服务的副本，确保在个别组件故障时系统仍能持续运行。

> 冗余是**高可用（HA）的物质基础**——没有多副本就无法故障转移。但仅有冗余还不够，仍需配合 [熔断](../circuit-break/README.md)（避免把请求路由到故障副本）和 [限流](../rate-limiting/README.md)（避免流量压垮健康副本）才能形成完整的高可用体系。
...
2. **一致性协议**
    ...
    | 对比维度 | Paxos | Raft | ZAB |
    |---------|-------|------|-----|
    | 理解难度 | 高（证明过程复杂） | 中（Leader 选举 + 日志复制直观） | 中（与 Raft 类似，专为 ZooKeeper 设计） |
    | 性能 | 理论最优，工程实现复杂 | 良好，主流实现成熟 | 良好，ZooKeeper 内置 |
    | 典型实现 | Chubby、Phaxio | etcd、Consul、TiKV | ZooKeeper |
...
2. **副本同步延迟**
    ...
    - **解决方案**：采用异步复制与同步复制结合的方式。例如，主节点写操作成功后立即返回响应，从节点异步同步数据，同时通过心跳检测确保从节点存活。典型阈值：同机房同步延迟 < 5 ms，跨地域异步延迟容忍 100–500 ms；超过阈值时触发告警并降级为只读。
...
- [集群](cluster/README.md) — 主从 / 多主集群模式与故障转移
- [多活](multi-site-active-active/README.md) — 同城双活 / 异地多活与流量调度
