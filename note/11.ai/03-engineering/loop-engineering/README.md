...
## 八、实战工具：Ralph Wiggum Loop

> 概念看完了，用什么工具落地？👉 [Ralph Wiggum Loop — Fresh Context 循环的实战工具](ralph-wiggum-loop.md)
>
> 开源 CLI 把 Agent 包在 `while` 循环里，每轮 fresh context，支持 Claude Code / Codex / Copilot 等 7 个 Provider。
...
| # | 文件 | 一句话定位 |
|---|------|-----------|
| 1 | [内置循环命令对比](builtin-loop-commands.md) 🆕 | `/goal` vs `/loop` vs Ralph Wiggum：架构差异 + 场景决策树 + 各工具支持矩阵 |
| 2 | [Ralph Wiggum Loop](ralph-wiggum-loop.md) | Fresh Context 循环的第三方 CLI 工具详细用法 |
| 3 | [Auto-Fix 修复策略](auto-fix-strategy.md) | 5 种按错误类型分发的修复策略 |
| 4 | [Verifier 设计](verifier-design.md) | 5 大客观验证源（测试/Lint/类型/构建/运行） |
| 5 | [IDE 实战案例](ide-case-studies.md) | 4 大 AI IDE Loop 工程实战案例 |
| 6 | [修复 Prompt 模板](fix-prompt-templates.md) | 4 类常用修复 Prompt 模板 |

### 外部关联

- 🆕 规范工具：[Agent Spec Tools](../agent-spec-tools/README.md) — Superpowers 子 Agent 编排也基于 Fresh Context
...
```
┌──────────────────────────────────────────────────────┐
│  用户输入：开发任务（如"实现用户登录接口")              │
│  ↓                                                    │
│  Agent 写代码 → 跑测试 → 失败 → 修复 → 跑测试 → 通过 │
│      ↑              ↓                                 │
│      └─── 循环 ←── 自动分析错误 → 修改代码             │
│  ↓                                                    │
│  通过后：commit 代码 / 提交 PR                        │
...
**关键**：Auto-Fix Loop 是 **Plan-Execute-Reflect** 范式的编程语言实例化。

### 2. 4 大子节点

| # | 章节 | 核心问题 |
|---|------|---------|
| 1 | [auto-fix-strategy 修复策略](auto-fix-strategy.md) | 5 种自动修复策略（按错误类型选择）|
| 2 | [verifier-design 验证器设计](verifier-design.md) | 测试 / 类型检查 / lint / 编译 / 运行时 5 大 Verifier |
| 3 | [ide-case-studies IDE 案例](ide-case-studies.md) | Claude Code / Cursor / Devin / Aider 4 大工具实战 |
| 4 | [fix-prompt-templates 修复 prompt 模板](fix-prompt-templates.md) | 4 类常见错误 + 对应 prompt 模板 |

### 3. 核心公式

```
Auto-Fix Loop = Task 描述 + Verifier（测试/lint/编译）+ Agent + 反馈循环 + 终止条件
        + Harness 兜底 + Context 记忆
...
### 4. 与现有 Loop Engineering 的关系

| 维度 | 综述（已有）| 实战（本节）|
|------|----------|----------|
| **视角** | 工程哲学 + Verifier + Harness | 编程场景的具体闭环 |
| **粒度** | 设计原则 + 4 反模式 | 5 修复策略 + 实战 prompt |
| **目标读者** | AI 工程师 / 架构师 | 业务开发 / AI Coding 工具用户 |
| **关联文档** | [README.md](README.md) | [auto-fix-strategy](auto-fix-strategy.md) 等 4 子 |

---

## 速查 · 关联资源

- **综述**：[README.md 总目录](README.md) —— Loop Engineering 综述（251 行）
- **实战 4 子**：[auto-fix-strategy](auto-fix-strategy.md) / [verifier-design](verifier-design.md) / [ide-case-studies](ide-case-studies.md) / [fix-prompt-templates](fix-prompt-templates.md)
