...
> 用开餐厅的故事，讲明白四十件技术大事。
> 从架构演进到 AI 智能体，从流量治理到 AI 私有化部署，从团队管理到多模态 AI 应用 —— 覆盖传统工程 + AI 时代 41 个核心主题。
> 一篇一个核心，篇篇独立又互相串联。详细目录见下文"系列全景"与"全部四十一篇文章"。
...
| **AI 时代主线** | 续集一 → 续集二-十七 → 番外七-八 | AI 工程师 / CTO |
...
> 以下按**叙事逻辑**排列（前传 → 续集一 → 正传 1-14 → 终章 → 番外 → 续集二 ~ 续集十七 → 番外七-八），而非文件编号顺序。每篇标题标注文件编号，方便检索。
>
> ⚠️ **2026 重要说明**：34/35/36 三篇已拆分为 34a/34b、35a/35b、36a/36b（结构性优化），旧重定向文件已移除。续集十三-十七（37-41）+ 番外七-八（42-43）为本期新增。
>
> 💡 **文件计数说明**：41 个叙事段中，34/35/36 各含 2 个子文件（a/b），故实际 `.md` 文件共 **44 个**（41 段 + 3 个拆分）。
...
- [术语表](./glossary.md) —— 310+ 词条速查，按 44 大主题分类
...
| AI 工程师 | 续集一 → 续集二 → 续集三 → 续集四 → 续集五 → 续集六 → 续集七 → 续集八 → 续集九 → 续集十 → 续集十一 → 续集十二 → 前传 → 正传 3 | Agent 全景 → 人机协作 → 组织转型 → AI 原生创业 → 自进化组织 → 幻觉护栏 → 认知债 → Harness → 致命三件套 → AI 评测 → 协议层 → AI 成本 → 架构基础 → 安全护栏 |
