...
本系列按**叙事逻辑**组织（**前传 → 续集一 → 正传 → 终章 → 番外 → 续集二十**），每篇标注文件编号方便检索。原编号 17 / 23 已合并到相邻主题（17 并入 18-distributed-puzzles，23 并入 24-database-migration），故编号从 16 直接跳到 18、从 22 跳到 24。**最新追加番外九「扩招 50 个厨师」（2026-07-06）**，总篇数升至 **48 篇**：45 个叙事段中，34 / 35 / 36 各含 2 个子文件（a / b），故实际 `.md` 文件共 **48 个**（45 段 + 3 个拆分）。
