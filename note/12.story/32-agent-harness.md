...
> **系列定位**：本篇是「阿明餐厅」系列的**续集八**。在[《当餐厅长出大脑》](./01-ai-agent-architecture.md)中，我们讲了 AI Agent 的 7 大模块（感知/记忆/规划/工具/协同/反馈/安全）。在[《Codebase 认知债》](./31-codebase-cognitive-debt.md)中，我们讲了 AI 时代代码可读性的隐形负债。但有个问题没回答：**怎么让多个 Agent 真正"好用"？** 答案是 Harness —— Agent 周围的脚手架、指挥中心、安全栏。**Agent 是发动机，Harness 是传动系统 + 仪表盘 + 安全气囊。**
...
**Context 治理是 Harness 最核心的工作**。Context 不是"喂得越多越好"，而是"喂得越准越好"。这呼应了[《Codebase 认知债》](./31-codebase-cognitive-debt.md)第五章的观点：**RAG 的质量上限 = 认知债的下限**。
...
这呼应了[《AI 的"黑暗料理"》](./30-ai-hallucination-safety.md)中的"三层护栏"和 HITL 设计，但 Harness 把护栏工程化、产品化、自动化。
...
**关键洞察**：单纯的向量检索召回率只有 60-70%。4 段式组合能达到 85%+，并且**误召回（噪声）少 50%**。这呼应了[《Codebase 认知债》](./31-codebase-cognitive-debt.md)第五章："RAG 的质量上限 = 认知债的下限" —— 检索质量好，Agent 输出才靠谱。
...
- [Codebase 认知债](./31-codebase-cognitive-debt.md) —— RAG 的质量上限 = 认知债的下限，Harness 治理 Context 必须先治理认知债
...
- [AI 的"黑暗料理"](./30-ai-hallucination-safety.md) —— 三层护栏的工程化实现
- [会自我进化的厨房](./29-self-evolving-company.md) —— Harness + Agent Loop = 自进化组织的"操作系统"
