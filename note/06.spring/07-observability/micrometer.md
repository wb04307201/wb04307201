...
> Micrometer Tracing 默认走 **Brave / Zipkin** 桥接（`micrometer-tracing-bridge-brave`）；Spring Boot 3.x+ 通过 `micrometer-tracing-bridge-otel` 提供与 OpenTelemetry SDK 的桥接，再由 **OTLP** 协议导出到任意兼容后端（Jaeger / Tempo / SigNoz / OTel Collector 等）。
