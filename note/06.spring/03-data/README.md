...
**Spring 数据层 = 事务管理 + 缓存抽象 + 数据访问（JDBC/JPA）**——本章聚焦"事务"和"缓存"两大支柱，前者保证数据一致性，后者提升性能。
...
graph TB
    ...
    class D root
    class TX,Cache,DAO,T1,T2,T3,T4,T5,T6,T6a,T6b,T6c,T6d,T6e,C1,C2,C3,C4 leaf
```
...
> 🚀 从 [事务管理总览](transaction/README.md) 开始
