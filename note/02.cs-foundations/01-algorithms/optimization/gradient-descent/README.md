...
| 变体 | 单次更新时间 | 空间复杂度 | 说明 |
|------|------------|-----------|------|
| **BGD** | O(n·d) | O(d) | 每次遍历全量 n 个样本，d 为参数维度 |
| **SGD** | O(d) | O(d) | 每次仅 1 个样本，但需更多迭代收敛 |
| **Mini-Batch** | O(b·d) | O(b+d) | b 为 batch size（通常 32-512） |
| **Adam** | O(b·d) | O(b+d) | 额外维护一阶/二阶动量（2×d） |

> **总收敛时间**：SGD 单次最快但迭代次数多；Adam 单次较慢但收敛步数少——综合效率 Adam > Mini-Batch SGD > SGD > BGD。
...
- **AI 训练**：[SFT 训练](../../../../09.ai-applications/fine-tuning/01-sft.md) / [RLHF PPO](../../../../09.ai-applications/fine-tuning/02-rlhf.md)
