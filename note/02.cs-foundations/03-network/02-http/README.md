...
---
## 引言：HTTP — 浏览器和服务器聊什么、怎么聊

HTTP 是 Web 应用的传输骨架。难点不在"GET 还是 POST"，而在**理解每一代协议对"队头阻塞 / 握手延迟 / 加密层 / 多路复用"的取舍**：HTTP/1.1 用文本+长连接解决早期问题，HTTP/2 用二进制分帧解决队头阻塞，HTTP/3 用 QUIC 把传输层换成 UDP 彻底消除握手延迟。本篇按版本演进，对比 HTTP/1.1 → HTTP/2 → HTTP/3 的关键差异。
